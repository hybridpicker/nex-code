...
function startServerMode() {
  ...
  const serverHooks = {
    onToken(text) {
      if (activeMsgId) { emit({ type: 'token', id: activeMsgId, text }); }
    },
    onThinkingToken() {
      // Thinking tokens are not forwarded to the webview (no display value)
    },
    onToolStart(toolName, args) {
      if (activeMsgId) { emit({ type: 'tool_start', id: activeMsgId, tool: toolName, args: args || {} }); }
    },
    onToolEnd(toolName, summary, ok) {
      if (activeMsgId) { emit({ type: 'tool_end', id: activeMsgId, tool: toolName, summary: summary || '', ok: !!ok }); }
    },
  };
    ...
    switch (msg.type) {
      case 'chat': {
        ...
        try {
          ...
          emit({ type: 'error', id: msgId, message: err?.message || String(err) });
        } finally {
          activeMsgId = null;
        }
        break;
