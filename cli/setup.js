...
async function runSetupWizard({ rl: replRL = null, force = false } = {}) {
  ...
  if (choice === "1") {
    ...
    console.log();
    console.log(
      `  ${D}Get your key: https://console.anthropic.com/settings/keys${R}`,
    );
  ...
  console.log();
  const addPerplexity = await ask(
    "Add Perplexity key for grounded web search? (y/N)",
    "n",
  );
  if (addPerplexity.toLowerCase() === "y") {
    console.log(
      `  ${D}Get your key: https://www.perplexity.ai/settings/api${R}`,
    );
  ...
  if (process.env.DEFAULT_PROVIDER) {
    try {
      ...
      /* registry not yet initialised (first-run) — will pick up env on init */
    }
  }
