...
function scoreDecompose(result, scenario) {
  ...
  const hasRequiredFields = result.every(
    (t) => t.task && typeof t.task === "string" && t.task.length > 0,
  );
  if (hasRequiredFields) score += 2;

  // No overlapping scopes (2 points)
  const allScopes = result.flatMap((t) =>
  ...
  const hasEstimates = result.every(
    (t) =>
      typeof t.estimatedCalls === "number" ||
      typeof t.estimatedSshCalls === "number",
  );
...
async function runScenario(scenario, modelSpec) {
  ...
  try {
    ...
    if (scenario.type === "synthesize") {
      const resultsText = scenario.subResults
        .map((r, i) => {
    ...
    return {
      score: 0,
      latencyMs: 0,
      error: `Unknown scenario type: ${scenario.type}`,
    };
  } catch (err) {
...
async function runOrchestratorBenchmark(opts = {}) {
  ...
  for (const model of models) {
    ...
    const avgDecompose =
      decomposeScores.length > 0
        ? decomposeScores.reduce((a, b) => a + b, 0) / decomposeScores.length
        : 0;
    const avgSynthesize =
      synthesizeScores.length > 0
        ? synthesizeScores.reduce((a, b) => a + b, 0) / synthesizeScores.length
        : 0;
    const avgLatency =
      modelResults.latencies.reduce((a, b) => a + b, 0) /
      modelResults.latencies.length;
...
function printResults(results) {
  ...
  for (const r of results) {
    ...
    console.log(
      `${medal} ${r.model.padEnd(25)} ${String(r.decompose + "/10").padEnd(10)} ${String(r.synthesize + "/10").padEnd(11)} ${latencyStr.padEnd(8)} ${C.bold}${r.overall}/10${C.reset}`,
    );
  }
