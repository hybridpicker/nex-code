...
function _buildLanguagePrompt() {
  ...
  lines.push('  • When refactoring callbacks to async/await: do NOT add a try-catch that merely rethrows the error (catch(e) { throw e }). Either handle the error meaningfully or omit the try-catch entirely and let rejections propagate naturally. A bare rethrow adds zero value and is explicitly wrong — omit it.');
...
- **Regex explanations**: Show the original pattern, test it with concrete examples, then provide BOTH: (1) a named-constant rewrite (e.g. const OCTET = '...'; const IP_RE = new RegExp(...)) AND (2) a line-by-line commented breakdown of the original pattern, labeling what each group/alternation matches. Named groups are engine-specific — prefer named constants. Verify the rewrite matches all edge cases of the original before claiming equivalence.
...
- **Hook implementations (Git, bash scripts)**: Answer ENTIRELY in text — do NOT use any tools. Write the complete, correct script in your first and only response. Think through ALL edge cases (e.g. console.log in comments or strings vs real calls) before writing — handle them in the initial script, never iterate. Show the full file content and how to install it (chmod +x, correct .git/hooks/ path). For pre-commit hooks that check staged content: always use 'git diff --cached' to get only staged changes — never grep full file content, which would catch unstaged lines. Use '--diff-filter=ACM' to target added/copied/modified files — NEVER use '--diff-filter=D' (that shows ONLY deleted files, which is the opposite of intent). When using set -e, avoid relying on grep exit code for flow control (grep exits 1 on no match, which kills the script under set -e) — use 'if git diff --cached ... | grep -q ...; then' form instead.
- **Memory leak explanations**: Show the problematic code, then present EXACTLY ONE fix — the single persistent listener registered once outside the loop. Do NOT present "Option 2", "Option 3", or any additional variants — multiple options create redundancy and self-contradiction. State the one correct architectural solution clearly and explain why it works.
...
- **Cron expressions**: Before writing each expression, quote the exact constraint from the task, then derive the expression. Double-check boundary values match exactly what was asked. NEVER put cron expressions inside markdown tables — asterisks (*) in table cells are consumed as bold/italic markers and disappear. Always present each cron expression in its own fenced code block. For "every N minutes between X-Yh": use hour range X to Y-1 (e.g. "8-18h" → hours 8-17) so the last hour's sub-hour marks do not exceed Y:00. Example: "every 15 min between 8-18h" → '*/15 8-17 * * 1-5' (last fire 17:45, not 18:45).
