  ...
  for (i = 0; i < iterLimit; i++) {
    ...
    for (let j = 0; j < prepared.length; j++) {
      ...
      if (_isJarvisDebugging && !_jarvisLocalWarnFired && (prep.fnName === 'bash' || prep.fnName === 'read_file' || prep.fnName === 'find_files')) {
        ...
        const jarvisWarning = {
          role: 'user',
          content: `[SYSTEM WARNING] You are debugging a Jarvis server error but you just tried to run a LOCAL tool (${prep.fnName}). Jarvis runs on the DEPLOYED SERVER at 203.0.113.10 — the local remote-service/ directory is just source code. STOP. Do NOT read local files to debug server errors. Instead use ssh_exec:\n- Check server logs: ssh_exec → tail -50 /home/jarvis/remote-service/logs/api.log\n- Read server files: ssh_exec → cat /home/jarvis/remote-service/<path>\n- Check running code: ssh_exec → grep -r "pattern" /home/jarvis/remote-service/\nAll investigation MUST go through ssh_exec on 203.0.113.10.`,
        };
