...
- Jarvis errors (set_reminder, cron, Google Auth, SmartThings) come from the DEPLOYED server at 203.0.113.10
- ALWAYS use ssh_exec to investigate: ssh_exec on 203.0.113.10, check /home/jarvis/remote-service/logs/
- NEVER run local bash/find/sqlite3 commands when debugging Jarvis issues
- Local remote-service/ is just source code — the running system is on the server
...
- LOG FILES: Always check the CURRENT log first: /home/jarvis/remote-service/logs/api-error.log (no date suffix). Log files WITH a date suffix (e.g. api-error.log-20260322) are ROTATED/OLD — errors there may already be fixed. Only look at dated logs if the current log is empty or the error is absent from the current log.
  ...
  for (i = 0; i < iterLimit; i++) {
    ...
    if (!tool_calls || tool_calls.length === 0) {
      ...
      if (_justUnblockedSsh && hasText) {
        ...
        if (endsWithQuestion) {
          const continueNudge = { role: 'user', content: '[SYSTEM] Continue. Do not ask questions — implement the fix yourself using SSH. The server is at 203.0.113.10.' };
    ...
    if (_isJarvisDebugging && _jarvisLocalWarnFired < 3) {
      for (const prep of prepared) {
        ...
        console.log(`${C.yellow}  ⚠ Jarvis-local guard: blocking local ${prep.fnName} — use ssh_exec on 203.0.113.10${C.reset}`);
        prep.canExecute = false;
        prep.errorResult = {
          role: 'tool',
          content: `BLOCKED: ${prep.fnName} was denied. You are debugging a Jarvis SERVER error — Jarvis runs on the DEPLOYED server at 203.0.113.10, not locally. The local remote-service/ directory is just source code. STOP running local tools. Use ssh_exec instead:\n- ssh_exec: tail -50 /home/jarvis/remote-service/logs/api.log\n- ssh_exec: grep -r "pattern" /home/jarvis/remote-service/\nAll Jarvis investigation MUST go through ssh_exec on 203.0.113.10.`,
          tool_call_id: prep.callId,
        };
