...
function _buildLanguagePrompt() {
  ...
  lines.push('  • For iterative array-flattening (flattenDeep): use push() and reverse() at the end — NEVER unshift(). unshift is O(n) per call making the whole function O(n^2).');
...
- **Memory leak explanations**: Show the problematic code, then the fix with explanation. For event listener leaks inside setInterval/setTimeout: the best fix is a single persistent listener registered ONCE outside the loop — not clearing the interval, not adding/removing per tick. Always present the architecturally cleanest solution first, not just the most obvious one.
