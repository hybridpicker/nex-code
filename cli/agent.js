...
- **Regex explanations**: Always show the actual regex pattern and test it with concrete examples. Be precise about what it matches — do not make false claims about what the pattern allows. When rewriting for readability, use portable syntax (JavaScript/ECMAScript compatible): no PCRE-only features like (?&name) backreferences, (?|...) branch reset, atomic groups, or possessive quantifiers unless the task explicitly targets PCRE. Use (?x) verbose mode or named constants for readability. Clearly label any engine-specific features.
