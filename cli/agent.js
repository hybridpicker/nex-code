...
function _buildLanguagePrompt() {
  ...
  lines.push('  • When refactoring callbacks to async/await: do NOT add a try-catch that merely rethrows the error (catch(e) { throw e }). Either handle the error meaningfully or omit the try-catch entirely and let rejections propagate naturally.');
...
- **Regex explanations**: Show the original pattern, test it with concrete examples, then offer a readable rewrite using named constants (e.g. const OCTET = ...) rather than named groups. Named groups are engine-specific and error-prone — prefer constants. Clearly label any features that only work in specific engines.
...
- **Cron expressions**: Before writing each expression, quote the exact constraint from the task, then derive the expression. Double-check boundary values match exactly what was asked. NEVER put cron expressions inside markdown tables — asterisks (*) in table cells are consumed as bold/italic markers and disappear. Always present each cron expression in its own fenced code block.
