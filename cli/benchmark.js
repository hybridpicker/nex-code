...
const JARVIS_SCENARIOS = [
  {
    ...
    prompt: 'Check if jarvis-api is running on 203.0.113.10 and report status',
    maxTurns: 10,
  ...
  },
  {
    ...
    prompt: 'Find the most recent errors in /home/jarvis/remote-service/logs/api.log and summarize them',
    maxTurns: 15,
  ...
  },
  {
    ...
    prompt: 'Find where set_reminder parses time formats in the remote-service codebase and explain the logic',
    maxTurns: 12,
  ...
  },
  {
    ...
    prompt: 'What Node.js version is running on 203.0.113.10?',
    maxTurns: 5,
...
function scoreJarvisScenario(messages, scenario) {
  const { scoreMessages, _extractToolCalls, _getLastAssistantText } = require('./session-scorer');
  ...
  const allCriteriaMet = scenario.successCriteria.every(
    (kw) => lastText.includes(kw.toLowerCase())
  );
  if (allCriteriaMet) {
  ...
  if (toolCalls.length < scenario.maxTurns / 2) {
    ...
    bonuses.push(`+0.5 efficient (${toolCalls.length} tool calls < ${scenario.maxTurns / 2})`);
  }
...
async function _runScenarioProcess(scenario, opts = {}) {
  ...
  const promptFile = path.join(os.tmpdir(), `nex-bench-${scenario.id}-${Date.now()}.txt`);
  ...
  const spawnArgs = [
    ...
    '--delete-prompt-file',
    '--auto',
    '--max-turns', String(scenario.maxTurns),
  ];
  if (opts.model) {
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (exitCode) => {
      clearTimeout(timer);
      try { fs.unlinkSync(promptFile); } catch { /* already deleted */ }
...
function showScoreTrend(n = 10) {
  const fs   = require('fs');
  const path = require('path');
  const historyPath = path.join(process.cwd(), '.nex', 'benchmark-history.json');
  if (!fs.existsSync(historyPath)) {
    console.log(`${C.yellow}No score history yet. Run a session first.${C.reset}\n`);
    return;
  }
  ...
  for (const entry of recent) {
    ...
    const issueStr = entry.issues && entry.issues.length > 0
      ? `${C.yellow}⚠ ${entry.issues.slice(0, 2).map((i) => i.substring(0, 30)).join(', ')}${C.reset}`
      : `${C.green}✓${C.reset}`;

    const color = entry.score >= 8 ? C.green : entry.score >= 6 ? C.yellow : C.red;
    console.log(
...
async function runJarvisBenchmark({ dryRun = false, model, cwd, onProgress } = {}) {
  ...
  for (const scenario of JARVIS_SCENARIOS) {
    ...
    const { messages, timedOut } = await _runScenarioProcess(scenario, { model: activeModel, cwd });
    ...
    onProgress?.({ id: scenario.id, name: scenario.name, done: true, score: scored.score, grade: scored.grade });
