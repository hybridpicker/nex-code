...
const INSTALL_MSG = 'Playwright is not installed. Install with:\n  npm install playwright && npx playwright install chromium\nThen restart nex-code.';
 ...
 */
async function browserNavigate(url, { timeout = 30000, waitFor = 'domcontentloaded' } = {}) {
  ...
  try {
    ...
    const text = await page.evaluate(() => {
      ...
      const remove = document.querySelectorAll('script,style,nav,footer,header,aside,[role=navigation]');
    ...
    const links = await page.evaluate(() =>
      Array.from(document.querySelectorAll('a[href]'))
        .slice(0, 30)
        .map(a => ({ text: (a.innerText || a.textContent || '').trim().substring(0, 80), href: a.href }))
 ...
 */
async function browserScreenshot(url, { width = 1280, height = 800, fullPage = false, timeout = 30000 } = {}) {
  const browser = await getBrowser();
  const page = await browser.newPage();
  try {
    ...
    const fs = require('fs');
    const tmpPath = pathMod.join(os.tmpdir(), `nex-screenshot-${Date.now()}.png`);
 ...
 */
async function browserFill(url, { selector, value, submit = false, timeout = 30000 } = {}) {
...
module.exports = { isPlaywrightAvailable, browserNavigate, browserScreenshot, browserClick, browserFill, closeBrowser, INSTALL_MSG };
