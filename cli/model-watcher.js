...
const KNOWN_MODELS_PATH = path.join(os.homedir(), '.nex-code', 'known-models.json');
...
function updateModelsEnv(summary) {
  ...
  if (current === winner.model) return { updated: false, reason: 'winner unchanged' };
...
function updateRoutingConfig(categoryWinners) {
  ...
  if (fs.existsSync(envPath)) {
    ...
    const ROUTING_BLOCK_START = '# ── Task-type routing (auto-updated by /benchmark) ──';
