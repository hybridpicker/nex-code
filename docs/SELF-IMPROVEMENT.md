...
```json
{
  ...
  "target_projects": ["nex-code", "server-agent", "sample-static-app"]
}
```
...
|---------|------|-------|-------|
| sample-static-app | Node | 4 | A11y, audio tests, validation |
...
**Files:**
- `remote-service/routes/nex-improve.js` — Node.js API
- `remote-service/web/templates/nex-improve/index.html` — Frontend
- `remote-service/web/chat/views.py` — Django view (`nex_improve_dashboard`)
- `remote-service/web/chat/api.py` — Django API proxy (`nex_improve_status_api`)
- `remote-service/web/chat/urls.py` — Routes (`/nex-improve/`, `/api/nex-improve/status/`)
