...
`);
      fs.writeFileSync(path.join(tmpDir, "package.json"), '{"name":"sample-react-app","version":"1.0.0"}\n');
      initGit(tmpDir);
    },
    evaluateFn(tmpDir) {
      const score = { taskCompletion: 0, editPrecision: 100, quality: 100 };
...
module.exports = router;
`);
      fs.writeFileSync(path.join(tmpDir, "package.json"), '{"name":"sample-node-app","version":"1.0.0","dependencies":{"express":"^4.18.0","better-sqlite3":"^9.0.0","cors":"^2.8.5"}}\n');
      initGit(tmpDir);
    },
    evaluateFn(tmpDir) {
      const score = { taskCompletion: 0, editPrecision: 100, quality: 100 };
