        ...
        if (oe && ce < 3) {
          for (let S of Pe)
            if (
              ...
              (re(
                `${$.yellow}  \u26A0 Jarvis-local guard: blocking local ${S.fnName} \u2014 use ssh_exec on 203.0.113.10${$.reset}`,
              ),
                (S.canExecute = !1),
                (S.errorResult = {
                  role: "tool",
                  content: `BLOCKED: ${S.fnName} denied \u2014 this is a server issue. Use ssh_exec on 203.0.113.10 instead.`,
                  tool_call_id: S.callId,
                }));
              break;
            }
