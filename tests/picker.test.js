...
describe('picker.js', () => {
  describe('pickFromList()', () => {
    ...
    it('uses default title and hint when options are empty', async () => {
      ...
      const writeCalls = process.stdout.write.mock.calls.map((c) => c[0]).join('');
    ...
    it('scrolls up when navigating above the visible window', async () => {
      ...
      const items = [];
      for (let i = 0; i < 10; i++) {
        items.push({ label: `Option ${i}`, value: `val-${i}`, isCurrent: i === 7 });
      }
  ...
  describe('showModelPicker()', () => {
    ...
    it('returns true and calls setActiveModel when a model is selected', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Switched to openai:gpt-4'));
    });
    ...
    it('skips providers with no models', async () => {
      ...
      const result = await promise;
      expect(result).toBe(true);
      expect(registry.setActiveModel).toHaveBeenCalledWith('anthropic:claude-3');
    });
