...
describe("providers/local.js", () => {
  ...
  describe("chat()", () => {
    ...
    it("throws when no model available", async () => {
      ...
      await expect(emptyProvider.chat([], [])).rejects.toThrow(
  ...
  describe("stream()", () => {
    ...
    it("throws when no model available", async () => {
      ...
      await expect(emptyProvider.stream([], [])).rejects.toThrow(
        "No local model available",
      );
    });
    ...
    it("processes remaining buffer on end", async () => {
      ...
      process.nextTick(() => {
        emitter.emit(
          "data",
          Buffer.from('{"message":{"content":"buf"},"done":true}'),
        );
        emitter.emit("end");
      });
      axios.post.mockResolvedValueOnce({ data: emitter });
