...
const { AnthropicProvider, ANTHROPIC_MODELS } = require('../../cli/providers/anthropic');
...
describe('providers/anthropic.js', () => {
  ...
  describe('formatMessages()', () => {
    ...
    it('converts tool role to user with tool_result', () => {
      const msgs = [{ role: 'tool', content: 'file1.txt\nfile2.txt', tool_call_id: 'tc1' }];
      const { messages } = provider.formatMessages(msgs);
      expect(messages[0].role).toBe('user');
      expect(messages[0].content).toEqual([
        { type: 'tool_result', tool_use_id: 'tc1', content: 'file1.txt\nfile2.txt' },
      ]);
    });
  ...
  describe('chat()', () => {
    ...
    it('throws when API key is missing', async () => {
      delete process.env.ANTHROPIC_API_KEY;
      await expect(provider.chat([], [])).rejects.toThrow('ANTHROPIC_API_KEY not set');
    });

    it('sends tools in Anthropic format', async () => {
      const tools = [
        { type: 'function', function: { name: 'bash', description: 'Run cmd', parameters: { type: 'object', properties: {} } } },
      ];
      axios.post.mockResolvedValueOnce({
      ...
      expect(axios.post.mock.calls[0][1].tools).toEqual([
        { name: 'bash', description: 'Run cmd', input_schema: { type: 'object', properties: {} } },
      ]);
    });
  ...
  describe('stream()', () => {
    function createSSEStream(events) {
      ...
      process.nextTick(() => {
        for (const event of events) {
          emitter.emit('data', Buffer.from(`data: ${JSON.stringify(event)}\n\n`));
        }
        emitter.emit('end');
      });
      return emitter;
    ...
    it('collects tool use from stream', async () => {
      ...
      const result = await provider.stream([{ role: 'user', content: 'list files' }], []);
    ...
    it('resolves on stream end without message_stop', async () => {
      const emitter = new EventEmitter();
      process.nextTick(() => {
        const data = JSON.stringify({ type: 'content_block_delta', delta: { type: 'text_delta', text: 'partial' } });
