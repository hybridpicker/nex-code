...
const { createSnapshot, listSnapshots, restoreSnapshot } = require('../cli/file-history');
...
describe('snapshot functions', () => {
  ...
  describe('listSnapshots()', () => {
    ...
    it('returns empty array when no nex snapshots in stash', () => {
      execSync.mockReturnValueOnce(Buffer.from(
        'stash@{0}: WIP on main: abc123 Some other commit\n'
      ));
      expect(listSnapshots('/tmp')).toEqual([]);
    });
    ...
    it('returns empty array on execSync error', () => {
      execSync.mockImplementationOnce(() => { throw new Error('git not found'); });
      expect(listSnapshots('/tmp')).toEqual([]);
    });
  });
  ...
  describe('restoreSnapshot()', () => {
    ...
    it('restores the most recent snapshot with "last"', () => {
      execSync
        .mockReturnValueOnce(Buffer.from('stash@{0}: On main: nex-snapshot-recent\n')) // listSnapshots
    ...
    it('restores by shortName', () => {
      execSync
        .mockReturnValueOnce(Buffer.from('stash@{0}: On main: nex-snapshot-my-snap\n')) // list
    ...
    it('returns error when target not found', () => {
      execSync.mockReturnValueOnce(Buffer.from('stash@{0}: On main: nex-snapshot-other\n'));
    ...
    it('returns error when git stash apply fails', () => {
      execSync
        .mockReturnValueOnce(Buffer.from('stash@{0}: On main: nex-snapshot-bad\n'))
