...
describe('registry.js', () => {
  ...
  describe('callStream() fallback', () => {
    it('retries on 429 rate limit error with fallback provider', async () => {
      ...
      jest.spyOn(ollama, 'stream').mockRejectedValueOnce(new Error('429 Too Many Requests'));
    ...
    it('retries on 503 server error', async () => {
      ...
      jest.spyOn(ollama, 'stream').mockRejectedValueOnce(new Error('503 Service Unavailable'));
      jest.spyOn(openai, 'isConfigured').mockReturnValue(true);
    ...
    it('throws non-retryable error immediately without retrying', async () => {
      ...
      jest.spyOn(ollama, 'stream').mockRejectedValueOnce(new Error('401 Unauthorized'));
      jest.spyOn(openai, 'isConfigured').mockReturnValue(true);
      jest.spyOn(openai, 'stream').mockResolvedValueOnce({ ok: true });

      await expect(registry.callStream([], [])).rejects.toThrow('401 Unauthorized');
    ...
    it('throws retryable error when last provider fails', async () => {
      ...
      jest.spyOn(ollama, 'stream').mockRejectedValueOnce(new Error('502 Bad Gateway'));
      jest.spyOn(openai, 'isConfigured').mockReturnValue(true);
      jest.spyOn(openai, 'stream').mockRejectedValueOnce(new Error('502 Bad Gateway'));

      await expect(registry.callStream([], [])).rejects.toThrow('502 Bad Gateway');
      expect(ollama.stream).toHaveBeenCalled();
      expect(openai.stream).toHaveBeenCalled();
    ...
    it('throws when no configured provider is available', async () => {
      ...
      await expect(registry.callStream([], [])).rejects.toThrow('No configured provider');
    });
  });
  ...
  describe('callChat() fallback', () => {
    it('retries on 429 rate limit error', async () => {
      ...
      jest.spyOn(ollama, 'chat').mockRejectedValueOnce(new Error('429 Too Many Requests'));
      jest.spyOn(openai, 'isConfigured').mockReturnValue(true);
    ...
    it('throws non-retryable error immediately', async () => {
      ...
      jest.spyOn(ollama, 'chat').mockRejectedValueOnce(new Error('401 Unauthorized'));
      jest.spyOn(openai, 'isConfigured').mockReturnValue(true);
      jest.spyOn(openai, 'chat').mockResolvedValueOnce({ content: 'ok' });

      await expect(registry.callChat([], [])).rejects.toThrow('401 Unauthorized');
    ...
    it('exhausts fallback chain and throws retryable error', async () => {
      ...
      jest.spyOn(ollama, 'chat').mockRejectedValueOnce(new Error('500 Internal Server Error'));
      jest.spyOn(openai, 'isConfigured').mockReturnValue(true);
      jest.spyOn(openai, 'chat').mockRejectedValueOnce(new Error('500 Internal Server Error'));

      await expect(registry.callChat([], [])).rejects.toThrow('500 Internal Server Error');
