...
jest.mock('../cli/ollama', () => ({
  getActiveModel: jest.fn().mockReturnValue({ id: 'qwen3-coder:480b', name: 'Qwen3 Coder', provider: 'ollama', contextWindow: 131072 }),
...
jest.mock('../cli/providers/registry', () => ({
  listProviders: jest.fn().mockReturnValue([
    { provider: 'ollama', configured: true, models: [{ id: 'qwen3-coder:480b', name: 'Qwen3 Coder', active: true }] },
    { provider: 'anthropic', configured: true, models: [{ id: 'claude-sonnet', name: 'Sonnet' }] },
  ]),
  getActiveProviderName: jest.fn().mockReturnValue('ollama'),
  getActiveModelId: jest.fn().mockReturnValue('qwen3-coder:480b'),
  getActiveModel: jest.fn().mockReturnValue({ id: 'qwen3-coder:480b', name: 'Qwen3 Coder', provider: 'ollama' }),
...
jest.mock('../cli/context-engine', () => ({
  getUsage: jest.fn().mockReturnValue({
    used: 95000, limit: 128000, percentage: 74.2,
    breakdown: { system: 500, conversation: 90000, toolResults: 4000, toolDefinitions: 500 },
    messageCount: 50,
  }),
}));
...
jest.mock('../cli/audit', () => ({
  getAuditSummary: jest.fn().mockReturnValue({
    totalCalls: 42, avgDuration: 150, successRate: 0.97,
...
describe('index-handlers.test.js — additional handler coverage', () => {
  ...
  describe('/benchmark', () => {
    ...
    it('shows benchmark data when results exist', async () => {
      ...
      fs.writeFileSync(resultFile, JSON.stringify({
        ...
        tasks: [{ passed: true, score: 0.9 }, { passed: true, score: 0.8 }],
        score: 85,
      }));
  ...
  describe('/optimize', () => {
    it('shows high context warning when > 75%', async () => {
      ...
      ctxEngine.getUsage.mockReturnValueOnce({
        used: 105000, limit: 128000, percentage: 82,
        breakdown: { system: 500, conversation: 100000, toolResults: 4000, toolDefinitions: 500 },
        messageCount: 80,
      });
    ...
    it('shows filling up warning when 50-75%', async () => {
      const ctxEngine = require('../cli/context-engine');
      ctxEngine.getUsage.mockReturnValueOnce({
        used: 80000, limit: 128000, percentage: 62.5,
        breakdown: { system: 500, conversation: 75000, toolResults: 4000, toolDefinitions: 500 },
        messageCount: 40,
      });
      await handleSlashCommand('/optimize');
      const output = logSpy.mock.calls.map(c => c[0]).join('\n');
    ...
    it('shows healthy context when < 50%', async () => {
      const ctxEngine = require('../cli/context-engine');
      ctxEngine.getUsage.mockReturnValueOnce({
        used: 5000, limit: 128000, percentage: 3.9,
        breakdown: { system: 500, conversation: 4000, toolResults: 400, toolDefinitions: 100 },
        messageCount: 5,
      });
      await handleSlashCommand('/optimize');
      const output = logSpy.mock.calls.map(c => c[0]).join('\n');
    ...
    it('shows pruning tip for many memories', async () => {
      ...
      const manyMemories = Array.from({ length: 35 }, (_, i) => ({
        key: `k${i}`, value: `v${i}`, updatedAt: new Date().toISOString(),
      }));
  ...
  describe('/learn', () => {
    it('handles learn with brain-only results', async () => {
      ...
      agent.getConversationMessages.mockReturnValueOnce(
        Array.from({ length: 8 }, (_, i) => ({ role: i % 2 === 0 ? 'user' : 'assistant', content: `msg${i}` }))
      );
      const learner = require('../cli/learner');
      learner.learnFromSession.mockResolvedValueOnce({
        skipped: false, applied: [], nexAdded: [],
      });
      learner.learnBrainFromSession.mockResolvedValueOnce({
        written: [{ name: 'patterns', reason: 'code patterns found', action: 'created' }],
      });
    ...
    it('handles learn with summary but no learnings', async () => {
      ...
      const learner = require('../cli/learner');
      learner.learnFromSession.mockResolvedValueOnce({
        skipped: false, applied: [], nexAdded: [], summary: 'Quick debug session',
      });
  ...
  describe('/init and /setup', () => {
    ...
    it('/setup calls runSetupWizard with force', async () => {
      ...
      expect(setup.runSetupWizard).toHaveBeenCalledWith(expect.objectContaining({ force: true }));
    });
  });
