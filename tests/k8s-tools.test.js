...
// ─── k8s_pods ────────────────────────────────────────────────
describe('k8s_pods', () => {
  beforeEach(() => { jest.clearAllMocks(); confirm.mockResolvedValue(true); });

  it('runs kubectl get pods -A when no namespace given', async () => {
    mockExecResolve('NAMESPACE  NAME       READY  STATUS   RESTARTS  AGE\ndefault    nginx-abc  1/1    Running  0         1d\n');
  ...
  it('uses -n flag when namespace is provided', async () => {
    mockExecResolve('NAME    READY  STATUS   RESTARTS  AGE\nnginx   1/1    Running  0         2h\n');
    await executeTool('k8s_pods', { namespace: 'production' }, { autoConfirm: true });
    const cmd = mockExec.mock.calls[0][0];
  ...
  it('adds label selector when label is given', async () => {
    ...
    await executeTool('k8s_pods', { label: 'app=nginx' }, { autoConfirm: true });
    const cmd = mockExec.mock.calls[0][0];
  ...
  it('prefixes with ssh when server is given', async () => {
    ...
    await executeTool('k8s_pods', { server: 'deploy@10.0.0.1' }, { autoConfirm: true });
    const cmd = mockExec.mock.calls[0][0];
  ...
  it('includes context flag when context is given', async () => {
    mockExecResolve('(no output)');
    await executeTool('k8s_pods', { context: 'prod-cluster' }, { autoConfirm: true });
    const cmd = mockExec.mock.calls[0][0];
  ...
  it('sanitizes server to prevent injection', async () => {
    mockExecResolve('ok');
    await executeTool('k8s_pods', { server: 'user@host; rm -rf /' }, { autoConfirm: true });
    const cmd = mockExec.mock.calls[0][0];
...
describe('k8s_logs', () => {
  ...
  it('fetches logs with default namespace and tail', async () => {
    ...
    const result = await executeTool('k8s_logs', { pod: 'nginx-abc' }, { autoConfirm: true });
    const cmd = mockExec.mock.calls[0][0];
  ...
  it('applies since flag', async () => {
    ...
    await executeTool('k8s_logs', { pod: 'api-pod', since: '30m' }, { autoConfirm: true });
    const cmd = mockExec.mock.calls[0][0];
  ...
  it('applies container flag', async () => {
    ...
    await executeTool('k8s_logs', { pod: 'api-pod', container: 'sidecar' }, { autoConfirm: true });
    const cmd = mockExec.mock.calls[0][0];
  ...
  it('truncates very long output', async () => {
    ...
    const result = await executeTool('k8s_logs', { pod: 'api-pod' }, { autoConfirm: true });
...
describe('k8s_exec', () => {
  ...
  it('requires pod and command', async () => {
    expect(await executeTool('k8s_exec', {}, { autoConfirm: true })).toMatch(/pod is required/);
    expect(await executeTool('k8s_exec', { pod: 'nginx' }, { autoConfirm: true })).toMatch(/command is required/);
  });

  it('asks for confirmation', async () => {
    mockExecResolve('output');
    await executeTool('k8s_exec', { pod: 'nginx', command: 'ls /app' }, { autoConfirm: true });
    expect(confirm).toHaveBeenCalledTimes(1);
  });

  it('cancels when user declines', async () => {
    confirm.mockResolvedValueOnce(false);
    const result = await executeTool('k8s_exec', { pod: 'nginx', command: 'rm -rf /' }, { autoConfirm: true });
  ...
  it('runs kubectl exec with -- sh -c', async () => {
    mockExecResolve('/app\n/etc\n');
    const result = await executeTool('k8s_exec', { pod: 'api-pod', command: 'ls /', namespace: 'prod' }, { autoConfirm: true });
    const cmd = mockExec.mock.calls[0][0];
...
describe('k8s_apply', () => {
  ...
  it('asks for confirmation before applying', async () => {
    ...
    await executeTool('k8s_apply', { file: 'deploy.yaml' }, { autoConfirm: true });
    expect(confirm).toHaveBeenCalledTimes(1);
  });

  it('cancels when user declines', async () => {
    confirm.mockResolvedValueOnce(false);
    const result = await executeTool('k8s_apply', { file: 'deploy.yaml' }, { autoConfirm: true });
    expect(result).toMatch(/CANCELLED/);
    expect(mockExec).not.toHaveBeenCalled();
  });
  ...
  it('uses --dry-run=client when dry_run=true (no confirmation)', async () => {
    ...
    await executeTool('k8s_apply', { file: 'deploy.yaml', dry_run: true }, { autoConfirm: true });
  ...
  it('applies with namespace flag when namespace given', async () => {
    ...
    await executeTool('k8s_apply', { file: 'svc.yaml', namespace: 'staging' }, { autoConfirm: true });
    const cmd = mockExec.mock.calls[0][0];
...
describe('k8s_rollout', () => {
  ...
  it('requires action and deployment', async () => {
    expect(await executeTool('k8s_rollout', {}, { autoConfirm: true })).toMatch(/action is required/);
    expect(await executeTool('k8s_rollout', { action: 'status' }, { autoConfirm: true })).toMatch(/deployment is required/);
  });

  it('runs rollout status without confirmation', async () => {
    mockExecResolve('deployment "nginx" successfully rolled out');
    await executeTool('k8s_rollout', { action: 'status', deployment: 'nginx' }, { autoConfirm: true });
    expect(confirm).not.toHaveBeenCalled();
    const cmd = mockExec.mock.calls[0][0];
  ...
  it('requires confirmation for restart', async () => {
    ...
    await executeTool('k8s_rollout', { action: 'restart', deployment: 'nginx' }, { autoConfirm: true });
    expect(confirm).toHaveBeenCalledTimes(1);
  });

  it('requires confirmation for undo', async () => {
    mockExecResolve('rolled back');
    await executeTool('k8s_rollout', { action: 'undo', deployment: 'api' }, { autoConfirm: true });
    expect(confirm).toHaveBeenCalledTimes(1);
  });

  it('cancels restart when user declines', async () => {
    confirm.mockResolvedValueOnce(false);
    const result = await executeTool('k8s_rollout', { action: 'restart', deployment: 'nginx' }, { autoConfirm: true });
    expect(result).toMatch(/CANCELLED/);
    expect(mockExec).not.toHaveBeenCalled();
  });

  it('runs history without confirmation', async () => {
    mockExecResolve('REVISION  CHANGE-CAUSE\n1         <none>\n2         update image');
    await executeTool('k8s_rollout', { action: 'history', deployment: 'api', namespace: 'prod' }, { autoConfirm: true });
    expect(confirm).not.toHaveBeenCalled();
    const cmd = mockExec.mock.calls[0][0];
...
describe('k8s tool definitions', () => {
  ...
  const k8sTools = ['k8s_pods', 'k8s_logs', 'k8s_exec', 'k8s_apply', 'k8s_rollout'];
