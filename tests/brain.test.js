...
beforeEach(() => {
  ...
  Object.keys(require.cache).forEach(k => {
    if (k.includes('brain.js') || k.includes('context-engine')) delete require.cache[k];
  });
});
...
describe('removeDocument', () => {
  it('deletes file and updates index', () => {
    const { writeDocument, removeDocument, getBrainDir, listDocuments } = getBrain();
...
describe('_keywordQuery', () => {
  ...
  it('respects minScore threshold', () => {
    const { writeDocument, _keywordQuery } = getBrain();
    writeDocument('doc', '# Document\n\nContent.');
    const results = _keywordQuery('document content', { topK: 3, minScore: 999 });
    expect(results).toEqual([]);
  });
});
...
describe('_fuseResults', () => {
  it('fuses keyword and semantic results via RRF', () => {
    ...
    const keyword = [{ name: 'doc-a', score: 5 }, { name: 'doc-b', score: 3 }];
    const semantic = [{ name: 'doc-b', score: 0.9 }, { name: 'doc-c', score: 0.7 }];
...
describe('brain_write tool', () => {
  ...
  beforeEach(() => {
    ...
    Object.keys(require.cache).forEach(k => {
      if (k.includes('/cli/tools') || k.includes('/cli/brain')) delete require.cache[k];
    });
  ...
  it('updates an existing document', async () => {
    ...
    const brainDir = path.join(tmpDir, '.nex', 'brain');
    const content = fs.readFileSync(path.join(brainDir, 'update-doc.md'), 'utf-8');
  ...
  it('appends to an existing document', async () => {
    ...
    const brainDir = path.join(tmpDir, '.nex', 'brain');
    const content = fs.readFileSync(path.join(brainDir, 'append-doc.md'), 'utf-8');
...
describe('integration: getBrainContext with multiple docs', () => {
  ...
  it('returns empty string when no docs match', async () => {
    const { writeDocument, getBrainContext } = getBrain();
    writeDocument('auth', '# Auth\n\nLogin flow.');
    const result = await getBrainContext('xyzxyzxyz totally unrelated zyxzyxzyx');
