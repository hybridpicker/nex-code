      ...
      response = {
        ...
        result: {
          content: [
            {
              type: "text",
              text: `result: ${JSON.stringify(request.params.arguments)}`,
            },
          ],
        },
      };
...
describe("mcp.js (connected server)", () => {
  it("connectServer initializes and discovers tools", async () => {
    const server = await connectServer("mock-srv", {
      command: "mock",
      args: [],
    });
  ...
  it("disconnectServer handles proc.kill() throwing", async () => {
    const server = await connectServer("kill-fail", {
      command: "mock",
      args: [],
    });
    server.proc.kill.mockImplementation(() => {
