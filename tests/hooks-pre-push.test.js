...
function matchesPattern(pattern, text) {
  try {
    execSync(`echo ${JSON.stringify(text)} | grep -Ei ${JSON.stringify(pattern)}`, {
      stdio: 'pipe',
    });
    return true;
  } catch {
...
function runHookWithDiff(diffContent, opts = {}) {
  ...
  try {
    ...
    const baseHash = execSync('git rev-parse HEAD', { cwd: tmpDir, stdio: 'pipe' }).toString().trim();

    // Add file with potential secret
    fs.writeFileSync(path.join(tmpDir, 'test-file.txt'), diffContent);
    execSync('git add . && git commit -m "add test file"', { cwd: tmpDir, stdio: 'pipe' });
    const headHash = execSync('git rev-parse HEAD', { cwd: tmpDir, stdio: 'pipe' }).toString().trim();
...
describe('hooks/pre-push', () => {
  ...
  describe('pattern matching', () => {
    ...
    describe('API Keys', () => {
      it('detects OpenAI sk- keys', () => {
        expect(matchesPattern('sk-[a-zA-Z0-9]{20,}', 'sk-abc123def456ghi789jkl012mno')).toBe(true);
      });
      ...
      it('detects AWS access keys', () => {
        expect(matchesPattern('AKIA[A-Z0-9]{16}', 'AKIAIOSFODNN7EXAMPLE')).toBe(true);
      });
      ...
      it('detects GitHub personal access tokens', () => {
        expect(matchesPattern('ghp_[a-zA-Z0-9]{36}', 'ghp_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghij')).toBe(true);
      });

      it('detects GitHub OAuth tokens', () => {
        expect(matchesPattern('gho_[a-zA-Z0-9]{36}', 'gho_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghij')).toBe(true);
      });

      it('detects GitHub App tokens', () => {
        expect(matchesPattern('ghs_[a-zA-Z0-9]{36}', 'ghs_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghij')).toBe(true);
      });

      it('detects Slack tokens', () => {
        expect(matchesPattern('xox[bpors]-[a-zA-Z0-9-]+', 'xoxb-123-456-abc')).toBe(true);
      });
    });
    ...
    describe('Private Keys', () => {
      it('detects RSA private key headers', () => {
        expect(matchesPattern('BEGIN (RSA|EC|DSA|OPENSSH|PGP) PRIVATE KEY', '-----BEGIN RSA PRIVATE KEY-----')).toBe(true);
      });

      it('detects EC private key headers', () => {
        expect(matchesPattern('BEGIN (RSA|EC|DSA|OPENSSH|PGP) PRIVATE KEY', '-----BEGIN EC PRIVATE KEY-----')).toBe(true);
      });

      it('detects OPENSSH private key headers', () => {
        expect(matchesPattern('BEGIN (RSA|EC|DSA|OPENSSH|PGP) PRIVATE KEY', '-----BEGIN OPENSSH PRIVATE KEY-----')).toBe(true);
      });

      it('does not match public keys', () => {
        expect(matchesPattern('BEGIN (RSA|EC|DSA|OPENSSH|PGP) PRIVATE KEY', '-----BEGIN PUBLIC KEY-----')).toBe(false);
      });
    });

    // Hardcoded secrets
    describe('Hardcoded Secrets', () => {
      const pattern = '(password|secret|token|api_key|apikey|api_secret|access_token|auth_token|credentials)\\s*[:=]\\s*[\'"][^\'"]{8,}';

      it('detects password = "longvalue"', () => {
        expect(matchesPattern(pattern, 'password = "mysecretpassword123"')).toBe(true);
      });

      it('detects token: "longvalue"', () => {
        expect(matchesPattern(pattern, "token: 'abcdefghijk123456'")).toBe(true);
      });

      it('detects api_key="longvalue"', () => {
        expect(matchesPattern(pattern, 'api_key="a1b2c3d4e5f6g7h8"')).toBe(true);
      });
      ...
      it('ignores unquoted values', () => {
        expect(matchesPattern(pattern, 'password = mysecretpassword123')).toBe(false);
      });
    });
    ...
    describe('SSH + IP', () => {
      it('detects ssh user@ip', () => {
        expect(matchesPattern('ssh\\s+.*@[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+', 'ssh root@192.168.1.100')).toBe(true);
      });

      it('detects ssh -i key user@ip', () => {
        expect(matchesPattern('ssh\\s+.*@[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+', 'ssh -i mykey.pem admin@10.0.0.1')).toBe(true);
      });

      it('does not match ssh with hostname', () => {
        expect(matchesPattern('ssh\\s+.*@[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+', 'ssh user@example.com')).toBe(false);
      });
    });

    // .env leaks
    describe('.env Leaks', () => {
      const pattern = '^[+].*\\b(API_KEY|SECRET_KEY|SECRET|TOKEN|PASSWORD|PRIVATE_KEY|ACCESS_KEY|AUTH_TOKEN|DB_PASSWORD|DATABASE_URL)\\s*=';
  ...
  describe('integration', () => {
    it('passes clean files without secrets', () => {
      const result = runHookWithDiff('Hello, this is a normal file.\nNo secrets here.\n');
      expect(result.exitCode).toBe(0);
    });

    it('blocks files containing an OpenAI key', () => {
      const result = runHookWithDiff('const key = "sk-abcdefghijklmnopqrstuvwxyz1234567890";\n');
      expect(result.exitCode).toBe(1);
      expect(result.output).toContain('SECRET DETECTED');
    });

    it('blocks files containing a private key header', () => {
      const result = runHookWithDiff('-----BEGIN RSA PRIVATE KEY-----\nMIIEpA...\n-----END RSA PRIVATE KEY-----\n');
      expect(result.exitCode).toBe(1);
      expect(result.output).toContain('SECRET DETECTED');
    });

    it('blocks files with hardcoded passwords', () => {
      const result = runHookWithDiff('const config = { password: "superSecretPass123!" };\n');
      expect(result.exitCode).toBe(1);
      expect(result.output).toContain('SECRET DETECTED');
    });
