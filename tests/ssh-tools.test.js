...
describe("ssh_exec", () => {
  it("executes command and returns stdout", async () => {
    ...
    const result = await executeTool(
      "ssh_exec",
      { server: "prod", command: "systemctl is-active nginx" },
  ...
  it("returns error on resolveProfile failure", async () => {
    mockResolveProfile.mockImplementation(() => {
      throw new Error('Unknown server: "nope". No profiles');
    });
    const { executeTool } = getTools();
    const result = await executeTool(
      "ssh_exec",
      { server: "nope", command: "ls" },
      { autoConfirm: true, silent: true },
    );
...
describe("service_manage", () => {
  ...
  it("runs status remotely via sshExec", async () => {
    ...
    const result = await executeTool(
      "service_manage",
      { server: "prod", service: "nginx", action: "status" },
      { autoConfirm: true, silent: true },
    );
    expect(result).toContain("nginx");
  ...
  it("returns error on unknown remote server", async () => {
    mockResolveProfile.mockImplementation(() => {
      throw new Error('Unknown server: "unknown". No profiles');
    });
    const { executeTool } = getTools();
    const result = await executeTool(
      "service_manage",
      { server: "unknown", service: "nginx", action: "restart" },
      { autoConfirm: true, silent: true },
    );
    expect(result).toMatch(/ERROR.*Unknown server/);
  });
...
describe("service_logs", () => {
  it("fetches logs locally", async () => {
    mockExecImpl.mockImplementation((cmd, opts, cb) =>
      cb(null, "Mar 11 10:00:00 nginx[123]: started", ""),
    );
    const { executeTool } = getTools();
    const result = await executeTool(
      "service_logs",
  ...
  it("fetches logs remotely via sshExec", async () => {
    ...
    const result = await executeTool(
      "service_logs",
      { server: "prod", service: "nginx", lines: 20 },
      { autoConfirm: true, silent: true },
    );
    expect(result).toContain("nginx");
    expect(mockSshExec).toHaveBeenCalled();
  ...
  it("passes --since flag when specified", async () => {
    ...
    await executeTool(
      "service_logs",
      { service: "nginx", since: "1 hour ago" },
      { autoConfirm: true, silent: true },
    );
