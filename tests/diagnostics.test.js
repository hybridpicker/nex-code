...
describe('runDiagnostics', () => {
  ...
  test('detects OpenAI API key', () => {
    const result = runDiagnostics('config.js', 'const key = "sk-abcdefghijklmnopqrstuvwx";');
    expect(result).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ severity: 'error', message: expect.stringContaining('OpenAI API Key') }),
      ])
    );
  });
  ...
  test('detects Anthropic API key', () => {
    ...
    expect(result).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ severity: 'error', message: expect.stringContaining('Anthropic API Key') }),
      ])
    );
  });
  ...
  test('detects GitHub token', () => {
    ...
    expect(result).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ severity: 'error', message: expect.stringContaining('GitHub Token') }),
      ])
    );
  });

  test('detects AWS access key', () => {
    const result = runDiagnostics('creds.yml', 'aws_access_key_id: AKIAIOSFODNN7EXAMPLE');
    expect(result).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ severity: 'error', message: expect.stringContaining('AWS Access Key') }),
      ])
    );
  });
  ...
  test('detects private key header', () => {
    ...
    expect(result).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ severity: 'error', message: expect.stringContaining('Private Key') }),
      ])
    );
  });

  test('detects database URL', () => {
    const result = runDiagnostics('config.js', 'const db = "postgres://user:pass@localhost:5432/mydb";');
    expect(result).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ severity: 'error', message: expect.stringContaining('Database URL') }),
      ])
    );
  });

  test('detects hardcoded secret', () => {
    const result = runDiagnostics('app.js', "password = 'supersecretpassword123';");
    expect(result).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ severity: 'error', message: expect.stringContaining('Hardcoded Secret') }),
      ])
    );
  });
  ...
  test('detects debugger statement with error severity in .js', () => {
    ...
    expect(result).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ severity: 'error', message: 'Found Debugger' }),
      ])
    );
  });
  ...
  test('detects console.log() with info severity', () => {
    ...
    expect(result).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ severity: 'info', message: 'Found Console Log' }),
      ])
    );
  });
  ...
  test('detects ANSI codes with custom message', () => {
    ...
    expect(result).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ severity: 'warn', message: expect.stringContaining('Avoid hardcoded ANSI') }),
      ])
    );
  });
  ...
  test('large file (>500 lines) triggers info diagnostic at line 0', () => {
    ...
    const largeFileHit = result.find(d => d.line === 0 && d.message.includes('Large file'));
  ...
  test('clean file returns no diagnostics', () => {
    const result = runDiagnostics('clean.js', 'function add(a, b) { return a + b; }');
