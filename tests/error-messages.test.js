...
Please retry the tool call with valid JSON arguments matching this schema.`;
    ...
    test('validation error should be clear', () => {
      const validationError = 'ERROR: Missing required parameter "path" for tool read_file';
    ...
    test('rate limit max retries exceeded should suggest action', () => {
      const msg = 'Rate limit: max retries (5) exceeded. Try again later or use /budget to check your limits.';
    ...
    test('network error max retries exceeded should suggest action', () => {
      const msg = 'Network error: max retries (3) exceeded. Check your connection and try again.';
    ...
    test('max iterations message should be clear and suggest action', () => {
      const maxIterMsg = '⚠ Max iterations (30) reached. The task may be too complex — try breaking it into smaller steps.';
  ...
  - src/utils/helpers.js`;
    ...
    test('binary file error should be clear', () => {
      const error = 'ERROR: /path/to/file.bin is a binary file (not readable as text)';
...
const x = 5;`;
    ...
    test('web fetch error should include URL', () => {
      const error = 'ERROR: Failed to fetch https://example.com: Network timeout';
    ...
    test('unknown task_list action error should list valid actions', () => {
      const error = 'ERROR: Unknown task_list action: delete. Use: create, update, get';
    ...
    test('bash blocked error should show pattern', () => {
      const error = 'BLOCKED: Command matches forbidden pattern: /rm\\s+-rf\\s+\\//';
    ...
    test('glob truncated warning should suggest narrowing', () => {
      const warning = '⚠ Results truncated at 200. Use a more specific pattern to narrow results.';
  ...
  describe('cli/providers/registry.js error messages', () => {
    test('budget limit error should show usage and solution', () => {
      const error = 'All providers are over budget. Use /budget to check limits or /budget <provider> off to remove a limit.';
  ...
  describe('cli/sub-agent.js error messages', () => {
    ...
    test('file locked error should suggest alternatives', () => {
      const error = "ERROR: File 'src/app.js' is locked by another sub-agent. Try a different approach or skip this file.";
  ...
  describe('cli/mcp.js error messages', () => {
    ...
    test('MCP server connection failed should include server name', () => {
      const error = "Failed to connect MCP server 'my-server': Connection refused";
