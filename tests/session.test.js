...
describe("session.js", () => {
  ...
  describe("saveSession()", () => {
    ...
    it("saves message data correctly", () => {
      ...
      const data = JSON.parse(
        fs.readFileSync(path.join(sessDir, "test-session.json"), "utf-8"),
      );
    ...
    it("preserves createdAt from meta", () => {
      ...
      const sessDir = path.join(tmpDir, ".nex", "sessions");
      const data = JSON.parse(
        fs.readFileSync(path.join(sessDir, "with-date.json"), "utf-8"),
      );
    ...
    it("handles default meta values", () => {
      ...
      const sessDir = path.join(tmpDir, ".nex", "sessions");
      const data = JSON.parse(
        fs.readFileSync(path.join(sessDir, "defaults.json"), "utf-8"),
      );
  ...
  describe("listSessions()", () => {
    ...
    it("includes metadata in listing", () => {
      session.saveSession("detailed", sampleMessages, {
        model: "gpt-4o",
        provider: "openai",
      });
      const list = session.listSessions();
