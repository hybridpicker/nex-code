...
describe("mcp.js", () => {
  ...
  describe("routeMCPCall()", () => {
    ...
    it("throws when server is not connected", async () => {
      disconnectAll();
      await expect(routeMCPCall("mcp_unknown_tool", {})).rejects.toThrow(
        "not connected",
      );
    });
  });
  ...
  describe("callTool()", () => {
    it("throws when server not connected", async () => {
      disconnectAll();
      await expect(callTool("no-server", "tool", {})).rejects.toThrow(
        "not connected",
      );
    });
  });
  ...
  describe("sendRequest write failure", () => {
    it("rejects when stdin.write throws", async () => {
      ...
      await expect(
        sendRequest(mockProc, "test/write-fail", {}, 5000),
