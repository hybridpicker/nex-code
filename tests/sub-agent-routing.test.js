...
describe("resolveSubAgentModel()", () => {
  ...
  it("uses explicit valid provider:model spec", () => {
    const mockProv = {
      isConfigured: () => true,
      getModel: (id) => (id === "kimi-k2.5" ? { id: "kimi-k2.5" } : null),
    };
...
describe("runSubAgent()", () => {
  ...
  it("passes overrideTier to filterToolsForModel", async () => {
    ...
    expect(filterToolsForModel).toHaveBeenCalledWith(expect.any(Array), "full");
  });
});
...
describe("isRetryableError()", () => {
  it("retries on 429 rate limit", () => {
    expect(isRetryableError(new Error("Request failed with status 429"))).toBe(
      true,
    );
  });
...
describe("executeSpawnAgents() depth guard", () => {
  ...
  it("at depth 1 uses ↳ prefix in labels", async () => {
    ...
    await executeSpawnAgents({ agents: [{ task: "Review auth module" }] }, 1);
