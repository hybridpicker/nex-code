...
describe("git.js", () => {
  ...
  describe("getMergeConflicts()", () => {
    ...
    it("detects merge conflicts (UU)", async () => {
      ...
      execSync('git add conflict.txt && git commit -m "add conflict.txt"', {
        cwd: tmpDir,
      });
      ...
      fs.writeFileSync(
        path.join(tmpDir, "conflict.txt"),
        "feature-change\n",
        "utf-8",
      );
      execSync('git add conflict.txt && git commit -m "feature change"', {
        cwd: tmpDir,
      });
      ...
      try {
        execSync("git merge feature", { cwd: tmpDir, stdio: "pipe" });
      } catch {}
