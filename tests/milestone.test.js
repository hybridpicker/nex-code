...
describe("_phaseName", () => {
  ...
  test("classifies >50% web tools as Research", () => {
    expect(_phaseName(counts({ web_search: 3, read_file: 1 }), 1)).toBe(
      "Research",
    );
  });

  test("classifies >50% explore tools as Exploration", () => {
    expect(_phaseName(counts({ read_file: 4, bash: 1 }), 1)).toBe(
      "Exploration",
    );
  });
  ...
  test("classifies >30% write tools as Implementation", () => {
    ...
    expect(_phaseName(counts({ write_file: 2, bash: 3 }), 2)).toBe(
      "Implementation",
    );
  });

  test("classifies >30% exec + <15% write as Verification", () => {
    expect(_phaseName(counts({ bash: 4, read_file: 3 }), 2)).toBe(
      "Verification",
    );
  });
  ...
  test("falls back to Phase N when no dominant type", () => {
    expect(
      _phaseName(
        counts({ bash: 1, write_file: 1, read_file: 1, web_search: 1 }),
        5,
      ),
    ).toBe("Phase 5");
  });
