...
describe('hooks.js', () => {
  ...
  describe('loadHookConfig()', () => {
    ...
    it('returns empty object when config has no hooks', () => {
      ...
      fs.writeFileSync(path.join(configDir, 'config.json'), JSON.stringify({ mcpServers: {} }));
      expect(loadHookConfig()).toEqual({});
    });
  ...
  describe('getHooksForEvent()', () => {
    ...
    it('finds hook script files in .nex/hooks/', () => {
      ...
      fs.writeFileSync(path.join(hooksDir, 'pre-tool'), '#!/bin/bash\necho ok', { mode: 0o755 });
  ...
  describe('executeHook()', () => {
    ...
    it('passes environment variables', () => {
      const result = executeHook('echo $NEX_TEST_VAR', { NEX_TEST_VAR: 'works' });
      expect(result.success).toBe(true);
  ...
  describe('runHooks()', () => {
    ...
    it('runs all hooks for an event', () => {
      ...
      fs.writeFileSync(
        path.join(configDir, 'config.json'),
        JSON.stringify({ hooks: { 'post-response': ['echo first', 'echo second'] } })
      );
    ...
    it('stops on pre-* hook failure', () => {
      ...
      fs.writeFileSync(
        path.join(configDir, 'config.json'),
        JSON.stringify({ hooks: { 'pre-tool': ['false', 'echo should-not-run'] } })
      );
