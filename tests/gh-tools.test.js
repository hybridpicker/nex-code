...
describe('GitHub Actions tools', () => {
  ...
  describe('tool definitions', () => {
    ...
    it('includes gh_run_list', () => {
      expect(TOOL_DEFINITIONS.some(t => t.function.name === 'gh_run_list')).toBe(true);
    });

    it('includes gh_run_view', () => {
      expect(TOOL_DEFINITIONS.some(t => t.function.name === 'gh_run_view')).toBe(true);
    });

    it('includes gh_workflow_trigger', () => {
      expect(TOOL_DEFINITIONS.some(t => t.function.name === 'gh_workflow_trigger')).toBe(true);
    });

    it('gh_run_list has no required params', () => {
      const def = TOOL_DEFINITIONS.find(t => t.function.name === 'gh_run_list');
      expect(def.function.parameters.required).toEqual([]);
    });

    it('gh_run_view requires run_id', () => {
      const def = TOOL_DEFINITIONS.find(t => t.function.name === 'gh_run_view');
      expect(def.function.parameters.required).toContain('run_id');
    });

    it('gh_workflow_trigger requires workflow', () => {
      const def = TOOL_DEFINITIONS.find(t => t.function.name === 'gh_workflow_trigger');
  ...
  describe('executeTool("gh_run_view")', () => {
    ...
    it('truncates long log output', async () => {
      ...
      const result = await executeTool('gh_run_view', { run_id: '123456', log: true });
  ...
  describe('executeTool("gh_workflow_trigger")', () => {
    it('triggers workflow after confirmation', async () => {
      ...
      resolveExec('');
      const result = await executeTool('gh_workflow_trigger', { workflow: 'ci.yml', branch: 'main' });
