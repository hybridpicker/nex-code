...
describe("skills.js", () => {
  ...
  describe("enableSkill() / disableSkill()", () => {
    ...
    it("disableSkill disables a loaded skill", () => {
      ...
      const config = JSON.parse(
        fs.readFileSync(path.join(nexDir, "config.json"), "utf-8"),
      );
    ...
    it("disableSkill is idempotent", () => {
      ...
      expect(config.skills.disabled.filter((n) => n === "style")).toHaveLength(
        1,
      );
    });
  });
  ...
  describe("installSkill()", () => {
    it("returns error for invalid git URL", async () => {
      const skills = getSkills();
      const result = await skills.installSkill(
  ...
  describe("searchSkills()", () => {
    ...
    beforeEach(() => {
      ...
      origAxios = undefined;
      try {
        origAxios = require.resolve("axios");
      } catch {}
    });
