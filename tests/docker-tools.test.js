...
describe('container_list', () => {
  it('lists running containers locally', async () => {
    mockExecImpl.mockImplementation((cmd, opts, cb) =>
      cb(null, 'CONTAINER ID   NAMES     IMAGE   STATUS   PORTS\nabc123   nginx   nginx:latest   Up 2 hours   80/tcp', ''));
    const { executeTool } = getTools();
    const result = await executeTool('container_list', {}, { autoConfirm: true, silent: true });
  ...
  it('lists all containers with all=true', async () => {
    ...
    const { executeTool } = getTools();
    await executeTool('container_list', { all: true }, { autoConfirm: true, silent: true });
  });

  it('lists containers on remote server', async () => {
    mockResolveProfile.mockReturnValue({ host: '1.2.3.4', user: 'jarvis' });
    mockSshExec.mockResolvedValue({ stdout: 'abc123   nginx   Up', stderr: '', exitCode: 0 });
    const { executeTool } = getTools();
    const result = await executeTool('container_list', { server: 'prod' }, { autoConfirm: true, silent: true });
    expect(result).toContain('nginx');
...
describe('container_logs', () => {
  it('fetches logs locally', async () => {
    ...
    const { executeTool } = getTools();
    const result = await executeTool('container_logs', { container: 'nginx' }, { autoConfirm: true, silent: true });
    expect(result).toContain('nginx: started');
  });

  it('returns error when container param is missing', async () => {
    const { executeTool } = getTools();
    const result = await executeTool('container_logs', {}, { autoConfirm: true, silent: true });
    expect(result).toMatch(/ERROR.*container/);
  });

  it('fetches logs remotely', async () => {
    mockResolveProfile.mockReturnValue({ host: '1.2.3.4', user: 'jarvis' });
    mockSshExec.mockResolvedValue({ stdout: 'log line from server', stderr: '', exitCode: 0 });
    const { executeTool } = getTools();
    const result = await executeTool('container_logs', { server: 'prod', container: 'app' }, { autoConfirm: true, silent: true });
  ...
  it('passes --since flag', async () => {
    ...
    const { executeTool } = getTools();
    await executeTool('container_logs', { container: 'nginx', since: '1h' }, { autoConfirm: true, silent: true });
  });
  ...
  it('passes --tail with custom lines count', async () => {
    ...
    const { executeTool } = getTools();
    await executeTool('container_logs', { container: 'nginx', lines: 100 }, { autoConfirm: true, silent: true });
  });
});
...
describe('container_exec', () => {
  it('executes read-only command locally without confirmation', async () => {
    mockExecImpl.mockImplementation((cmd, opts, cb) => cb(null, '/etc/nginx/nginx.conf content', ''));
    const { executeTool } = getTools();
    const result = await executeTool('container_exec', { container: 'nginx', command: 'cat /etc/nginx/nginx.conf' }, { autoConfirm: true, silent: true });
    expect(result).toContain('nginx.conf');
  });

  it('returns error when container param is missing', async () => {
    const { executeTool } = getTools();
    const result = await executeTool('container_exec', { command: 'ls' }, { autoConfirm: true, silent: true });
    expect(result).toMatch(/ERROR.*container/);
  });

  it('returns error when command param is missing', async () => {
    const { executeTool } = getTools();
    const result = await executeTool('container_exec', { container: 'nginx' }, { autoConfirm: true, silent: true });
    expect(result).toMatch(/ERROR.*command/);
  });

  it('executes command on remote server via sshExec', async () => {
    mockResolveProfile.mockReturnValue({ host: '1.2.3.4', user: 'jarvis' });
    mockSshExec.mockResolvedValue({ stdout: 'uid=0(root)', stderr: '', exitCode: 0 });
    const { executeTool } = getTools();
    const result = await executeTool('container_exec', { server: 'prod', container: 'app', command: 'id' }, { autoConfirm: true, silent: true });
...
describe('container_manage', () => {
  it('inspects a container locally (read-only, no confirm)', async () => {
    mockExecImpl.mockImplementation((cmd, opts, cb) => cb(null, '[{"Id":"abc"}]', ''));
    const { executeTool } = getTools();
    const result = await executeTool('container_manage', { container: 'nginx', action: 'inspect' }, { autoConfirm: false, silent: true });
  ...
  it('stops a container locally', async () => {
    ...
    const { executeTool } = getTools();
    const result = await executeTool('container_manage', { container: 'nginx', action: 'stop' }, { autoConfirm: true, silent: true });
    expect(result).toContain('nginx');
  });
  ...
  it('restarts a container remotely', async () => {
    ...
    const { executeTool } = getTools();
    const result = await executeTool('container_manage', { server: 'prod', container: 'app', action: 'restart' }, { autoConfirm: true, silent: true });
    expect(mockSshExec).toHaveBeenCalled();
  });

  it('returns error for invalid action', async () => {
    const { executeTool } = getTools();
    const result = await executeTool('container_manage', { container: 'nginx', action: 'explode' }, { autoConfirm: true, silent: true });
    expect(result).toMatch(/ERROR.*invalid action/);
  });

  it('returns error when container param is missing', async () => {
    const { executeTool } = getTools();
    const result = await executeTool('container_manage', { action: 'stop' }, { autoConfirm: true, silent: true });
    expect(result).toMatch(/ERROR.*container/);
  });
  ...
  it('uses "rm" command for remove action', async () => {
    ...
    const { executeTool } = getTools();
    await executeTool('container_manage', { container: 'old-container', action: 'remove' }, { autoConfirm: true, silent: true });
  });
});
...
describe('deploy', () => {
  it('rsyncs local path to remote server', async () => {
    ...
    const { executeTool } = getTools();
    const result = await executeTool('deploy', { server: 'prod', local_path: 'dist/', remote_path: '/var/www/app' }, { autoConfirm: true, silent: true });
    expect(result).toContain('Deployed');
  });

  it('runs deploy_script after sync', async () => {
    mockResolveProfile.mockReturnValue({ host: '1.2.3.4', user: 'jarvis' });
    mockExecImpl.mockImplementation((cmd, opts, cb) => cb(null, 'sent 512 bytes', ''));
    mockSshExec.mockResolvedValue({ stdout: 'gunicorn restarted', stderr: '', exitCode: 0 });
    const { executeTool } = getTools();
  ...
  it('returns dry run output without executing', async () => {
    ...
    const { executeTool } = getTools();
    const result = await executeTool('deploy', {
      server: 'prod', local_path: 'dist/', remote_path: '/var/www/app', dry_run: true,
    }, { autoConfirm: true, silent: true });
  ...
  it('excludes specified paths', async () => {
    ...
    await executeTool('deploy', {
      server: 'prod', local_path: 'dist/', remote_path: '/var/www/app', exclude: ['node_modules', '.env'],
    }, { autoConfirm: true, silent: true });
  });

  it('returns error when server param is missing', async () => {
    const { executeTool } = getTools();
    const result = await executeTool('deploy', { local_path: 'dist/', remote_path: '/var/www' }, { autoConfirm: true, silent: true });
    expect(result).toMatch(/ERROR.*server/);
  });

  it('returns error when local_path is missing', async () => {
    const { executeTool } = getTools();
    const result = await executeTool('deploy', { server: 'prod', remote_path: '/var/www' }, { autoConfirm: true, silent: true });
    expect(result).toMatch(/ERROR.*local_path/);
  });

  it('returns error when remote_path is missing', async () => {
    const { executeTool } = getTools();
    const result = await executeTool('deploy', { server: 'prod', local_path: 'dist/' }, { autoConfirm: true, silent: true });
  ...
  it('returns error on rsync failure', async () => {
    ...
    const { executeTool } = getTools();
    const result = await executeTool('deploy', { server: 'prod', local_path: 'dist/', remote_path: '/var/www' }, { autoConfirm: true, silent: true });
