...
jest.mock('../cli/providers/registry', () => ({
  callStream: jest.fn(),
  getActiveModel: jest.fn().mockReturnValue({ id: 'test', name: 'Test', provider: 'ollama' }),
...
jest.mock('../cli/context', () => ({ gatherProjectContext: jest.fn().mockReturnValue('') }));
jest.mock('../cli/context-engine', () => ({
  fitToContext: jest.fn().mockImplementation(async (m) => ({ messages: m, compressed: false })),
  getUsage: jest.fn().mockReturnValue({ used: 0, limit: 128000, percentage: 0 }),
...
jest.mock('../cli/planner', () => ({ isPlanMode: jest.fn().mockReturnValue(false), getPlanModePrompt: jest.fn().mockReturnValue(''), PLAN_MODE_ALLOWED_TOOLS: [], setPlanContent: jest.fn() }));
...
jest.mock('../cli/tool-tiers', () => ({ filterToolsForModel: jest.fn().mockImplementation((t) => t), getModelTier: jest.fn().mockReturnValue('full'), PROVIDER_DEFAULT_TIER: {} }));
...
jest.mock('../cli/tasks', () => ({ setOnChange: jest.fn(), clearTasks: jest.fn() }));
...
describe('web_search with Perplexity', () => {
  ...
  it('uses DuckDuckGo when PERPLEXITY_API_KEY is not set', async () => {
    ...
    const mockHtml = '<a class="result__a" href="/l/?uddg=https%3A%2F%2Fexample.com&rut=x">Test Result</a>';
  ...
  it('uses Perplexity when PERPLEXITY_API_KEY is set', async () => {
    ...
    const result = await executeTool('web_search', { query: 'react hooks tutorial' });
  ...
  it('includes citations in Perplexity results', async () => {
    ...
    jest.spyOn(axios, 'post').mockResolvedValueOnce({
      data: {
        ...
        citations: ['https://cite1.com', 'https://cite2.com', 'https://cite3.com'],
      },
    });
...
describe('OpenAIProvider._formatSingleMessage() with images', () => {
  ...
  it('passes through data: URLs as-is', () => {
    const msg = {
      role: 'user',
      content: [
        { type: 'image', media_type: 'image/png', data: 'data:image/png;base64,xyz==' },
      ],
    };
    const result = provider._formatSingleMessage(msg);
    const imgBlock = result.content.find(b => b.type === 'image_url');
