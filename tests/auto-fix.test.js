...
const { autoFixPath, autoFixEdit, enrichBashError, executeTool } = require('../cli/tools');
...
describe('autoFixEdit()', () => {
  ...
  it('auto-fixes single character typo', () => {
    ...
    const result = autoFixEdit(content, 'const helo = "world";', 'const hello = "earth";');
  ...
  it('auto-fixes minor whitespace + typo within threshold', () => {
    ...
    const result = autoFixEdit(content, 'function doSomethng() {', 'function doNothing() {');
    expect(result).not.toBeNull();
    expect(result.autoFixed).toBe(true);
  ...
  it('returns null for large mismatch (>5% distance)', () => {
    const content = 'const hello = "world";';
    const result = autoFixEdit(content, 'let goodbye = "moon";', 'something else');
    expect(result).toBeNull();
  });
  ...
  it('preserves multi-line context in auto-fix', () => {
    ...
    const result = autoFixEdit(content, 'const fo = "bar";', 'const foo = "baz";');
    expect(result).not.toBeNull();
...
describe('enrichBashError()', () => {
  ...
  it('adds hint for "command not found"', () => {
    const result = enrichBashError('bash: foo: command not found', 'foo --version');
  ...
  it('adds hint for Python not found', () => {
    const result = enrichBashError('bash: python3: command not found', 'python3 --version');
    expect(result).toContain('HINT');
    expect(result).toContain('Python');
  });

  it('adds hint for MODULE_NOT_FOUND', () => {
    const result = enrichBashError("Cannot find module 'express'", 'node app.js');
    expect(result).toContain('HINT');
    expect(result).toContain('npm install express');
  });

  it('adds hint for relative module not found', () => {
    const result = enrichBashError("Cannot find module './missing'", 'node app.js');
    expect(result).toContain('HINT');
    expect(result).toContain('import path');
  });

  it('adds hint for permission denied', () => {
    const result = enrichBashError('Error: EACCES: permission denied, open /etc/test', 'cat /etc/test');
    expect(result).toContain('HINT');
  ...
  it('adds hint for TypeScript errors', () => {
    const result = enrichBashError("src/app.ts(42,5): error TS2345: Argument of type 'string' is not assignable", 'npx tsc');
    expect(result).toContain('HINT');
    expect(result).toContain('TypeScript');
  });

  it('adds hint for Jest test failures', () => {
    const result = enrichBashError('Test Suites: 2 failed, 10 passed, 12 total\nTests: 5 failed', 'npm test');
    expect(result).toContain('HINT');
...
describe('Integrated auto-fix in tools', () => {
  ...
  describe('read_file auto-fix path', () => {
    it('auto-finds file when path has wrong extension', async () => {
      ...
      const result = await executeTool('read_file', { path: path.join(toolDir, 'config.js') }, { autoConfirm: true, silent: true });
