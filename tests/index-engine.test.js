const { extractDefinitions, isIndexValid, searchIndex, getFileIndex, findFileInIndex } = require('../cli/index-engine');
...
describe('Content Index', () => {
  describe('extractDefinitions', () => {
    ...
    test('extracts exported function declarations', () => {
      ...
      const defs = extractDefinitions(code, '.js');
      expect(defs).toContainEqual({ type: 'function', name: 'fetchData', line: 1 });
    });

    test('extracts exported async function declarations', () => {
      const code = 'export async function loadData() {}';
      const defs = extractDefinitions(code, '.js');
      expect(defs).toContainEqual({ type: 'function', name: 'loadData', line: 1 });
    });

    test('extracts exported class declarations', () => {
      const code = 'export class MyComponent {}';
      const defs = extractDefinitions(code, '.js');
      expect(defs).toContainEqual({ type: 'class', name: 'MyComponent', line: 1 });
    });
    ...
    test('extracts let/var arrow functions', () => {
      ...
      const defs = extractDefinitions(code, '.js');
      expect(defs).toContainEqual({ type: 'function', name: 'handler', line: 1 });
      expect(defs).toContainEqual({ type: 'function', name: 'process', line: 2 });
    });

    test('extracts async arrow functions', () => {
      const code = 'const fetchItem = async (id) => { return id; };';
      const defs = extractDefinitions(code, '.js');
      expect(defs).toContainEqual({ type: 'function', name: 'fetchItem', line: 1 });
    });
    ...
    test('works with .mjs and .cjs extensions', () => {
      ...
      expect(extractDefinitions(code, '.mjs')).toContainEqual({ type: 'function', name: 'helper', line: 1 });
      expect(extractDefinitions(code, '.cjs')).toContainEqual({ type: 'function', name: 'helper', line: 1 });
    });
    ...
    test('extracts Go method receiver functions', () => {
      ...
      const defs = extractDefinitions(code, '.go');
      expect(defs).toContainEqual({ type: 'function', name: 'Listen', line: 1 });
    });
    ...
    test('works with .jsx extension', () => {
      ...
      expect(defs).toContainEqual({ type: 'function', name: 'Component', line: 1 });
    });
    ...
    test('extracts Python async def', () => {
      ...
      const defs = extractDefinitions(code, '.py');
      expect(defs).toContainEqual({ type: 'function', name: 'fetch_data', line: 1 });
    });
  ...
  describe('buildContentIndex + searchContentIndex', () => {
    ...
    test('buildContentIndex indexes JS files and searchContentIndex finds them', async () => {
      ...
      // Create a JS file
      fs.writeFileSync(path.join(tmpDir, 'app.js'), 'function myFunction() {}\nclass MyClass {}');
    ...
    test('searchContentIndex filters by type', async () => {
      ...
      fs.writeFileSync(path.join(tmpDir, 'lib.js'), 'function foo() {}\nclass Foo {}');
      await ie.refreshIndex(tmpDir);
    ...
    test('searchContentIndex sorts exact matches first', async () => {
      ...
      fs.writeFileSync(path.join(tmpDir, 'sort.js'), 'function fooBar() {}\nfunction foo() {}');
      await ie.refreshIndex(tmpDir);
      await ie.buildContentIndex(tmpDir);
