...
const mockPage = {
  ...
  getByText: jest.fn().mockReturnValue({ first: jest.fn().mockReturnValue({ click: jest.fn().mockResolvedValue(undefined) }) }),
  locator: jest.fn().mockReturnValue({ first: jest.fn().mockReturnValue({ click: jest.fn().mockResolvedValue(undefined) }) }),
...
jest.mock('../cli/diff', () => ({
  showClaudeDiff: jest.fn(), showClaudeNewFile: jest.fn(),
  showEditDiff: jest.fn(), confirmFileChange: jest.fn().mockResolvedValue(true),
}));
...
describe('Browser tool definitions', () => {
  ...
  it('includes browser_open', () => {
    expect(TOOL_DEFINITIONS.some(t => t.function.name === 'browser_open')).toBe(true);
  });

  it('includes browser_screenshot', () => {
    expect(TOOL_DEFINITIONS.some(t => t.function.name === 'browser_screenshot')).toBe(true);
  });

  it('includes browser_click', () => {
    expect(TOOL_DEFINITIONS.some(t => t.function.name === 'browser_click')).toBe(true);
  });

  it('includes browser_fill', () => {
    expect(TOOL_DEFINITIONS.some(t => t.function.name === 'browser_fill')).toBe(true);
  });

  it('browser_open requires url', () => {
    const def = TOOL_DEFINITIONS.find(t => t.function.name === 'browser_open');
    expect(def.function.parameters.required).toContain('url');
  });

  it('browser_screenshot requires url', () => {
    const def = TOOL_DEFINITIONS.find(t => t.function.name === 'browser_screenshot');
    expect(def.function.parameters.required).toContain('url');
  });

  it('browser_fill requires url, selector, value', () => {
    const def = TOOL_DEFINITIONS.find(t => t.function.name === 'browser_fill');
    expect(def.function.parameters.required).toContain('url');
...
describe('browserNavigate()', () => {
  ...
  beforeEach(() => {
    ...
    jest.mock('playwright', () => mockPlaywright, { virtual: true });
    jest.mock('fs', () => ({ ...jest.requireActual('fs'), writeFileSync: jest.fn(), existsSync: jest.fn().mockReturnValue(true) }));
    mockPage.evaluate
      .mockResolvedValueOnce('Main content text')  // text
      .mockResolvedValueOnce([{ text: 'Click me', href: 'https://example.com/page' }]); // links
...
describe('browserScreenshot()', () => {
  ...
  it('sets viewport size', async () => {
    await browserScreenshot('https://example.com', { width: 1920, height: 1080 });
...
describe('browserClick()', () => {
  ...
  it('clicks by text and returns new URL', async () => {
    const result = await browserClick('https://example.com', { text: 'Submit' });
  ...
  it('throws when neither selector nor text provided', async () => {
    await expect(browserClick('https://example.com', {})).rejects.toThrow('selector or text');
  });
});
...
describe('browserFill()', () => {
  ...
  it('fills field without submitting', async () => {
    const result = await browserFill('https://example.com', { selector: '#email', value: 'test@example.com', submit: false });
  ...
  it('presses Enter when submit:true', async () => {
    await browserFill('https://example.com', { selector: '#email', value: 'test@example.com', submit: true });
    expect(mockPage.keyboard.press).toHaveBeenCalledWith('Enter');
  });

  it('throws when selector missing', async () => {
    await expect(browserFill('https://example.com', { value: 'test' })).rejects.toThrow();
  });
});
