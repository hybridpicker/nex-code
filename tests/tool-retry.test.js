...
describe("Tool Call Retry with Schema Hints", () => {
  ...
  beforeEach(() => {
    ...
    require("../cli/tool-validator").validateToolArgs.mockReturnValue({
      valid: true,
    });
  });
  ...
  it("sends schema hint when tool args are unparseable string", async () => {
    ...
    const toolMsg = msgs.find(
      (m) =>
        m.role === "tool" && m.content.includes("Malformed tool arguments"),
    );
    expect(toolMsg).toBeDefined();
...
describe("parseToolArgs extended strategies", () => {
  ...
  it("extracts JSON from surrounding text", () => {
    expect(parseToolArgs('sure, here: {"command":"ls"} ok')).toEqual({
      command: "ls",
    });
  });
  ...
  it("fixes multiple unquoted keys", () => {
    expect(parseToolArgs('{path: "test.js", line_start: 1}')).toEqual({
      path: "test.js",
      line_start: 1,
    });
  });
