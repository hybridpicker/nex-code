...
export type AgentEvent =
  ...
  | { type: 'confirm_request'; id: string; question: string; tool: string; critical: boolean }
  | { type: 'done';            id: string }
...
export class AgentProcess extends EventEmitter {
  ...
  spawn(executable: string, model: string, cwd: string, env: Record<string, string>) {
    this.kill();
    this.buffer = '';
  ...
  send(text: string) {
    ...
    this.child.stdin.write(JSON.stringify({ type: 'chat', id: msgId, text }) + '\n');
    return msgId;
  }

  confirm(id: string, answer: boolean) {
    if (!this.child?.stdin) { return; }
    this.child.stdin.write(JSON.stringify({ type: 'confirm', id, answer }) + '\n');
  }
