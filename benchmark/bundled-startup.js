...
async function measureStartup(iterations = 10) {
  ...
  console.log(
    `${C.bold}${C.blue}Nex Code — Bundled Startup Benchmark${C.reset}`,
  );
  console.log(
    `${C.gray}Measuring dist/nex-code.js module load time (${iterations} runs)...${C.reset}\n`,
  );
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    process.stdout.write(
      `${C.gray}  Run ${i + 1}/${iterations}: ${formatMs(times[times.length - 1])}${C.reset}\r`,
    );
  }
  ...
  console.log(
    `  ${C.bold}Median:${C.reset}   ${formatMs(median).padStart(10)}`,
  );
  ...
  const target = 50;
  const status =
    avg <= target ? `${C.green}✓ PASS${C.reset}` : `${C.red}✗ FAIL${C.reset}`;

  console.log(
    `\n${C.bold}${C.green}╔═══════════════════════════════════════════════════════════╗${C.reset}`,
  );
  console.log(
    `${C.bold}${C.green}║${C.reset}  ${C.bold}BUNDLED STARTUP SUMMARY${C.reset}                                  ${C.bold}${C.green}║${C.reset}`,
  );
  console.log(
    `${C.bold}${C.green}╠═════════���═════════════════════════════════════════════════╣${C.reset}`,
  );
  console.log(
    `${C.bold}${C.green}║${C.reset}  Average Startup: ${C.bold}${formatMs(avg).padStart(10)}${C.reset}  (Target: ${target}ms)  ${status}  ${C.bold}${C.green}║${C.reset}`,
  );
  console.log(
    `${C.bold}${C.green}╚═══════════════════════════════════════════════════════════╝${C.reset}`,
  );
