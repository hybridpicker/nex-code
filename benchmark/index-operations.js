...
async function runBenchmarks() {
  console.log(
    `${C.bold}${C.blue}Nex Code — Index-Based File Operations Benchmark${C.reset}`,
  );
  console.log(
    `${C.gray}Comparing Ripgrep index vs traditional fs.walk...${C.reset}\n`,
  );
  ...
  console.log(
    `${C.bold}Ripgrep Available:${C.reset} ${hasRipgrep ? C.green + "Yes" + C.reset : C.red + "No" + C.reset}\n`,
  );

  if (!hasRipgrep) {
    console.log(
      `${C.yellow}⚠ Ripgrep not found. Install for optimal performance:${C.reset}`,
    );
  ...
  console.log(
    `  ${C.bold}fs.walk:${C.reset} ${formatMs(fsWalkResult.avg)} (found ${fsWalkFiles.length} files)\n`,
  );
  ...
  if (hasRipgrep) {
    ...
    console.log(
      `  ${C.bold}rg --files:${C.reset} ${formatMs(rgResult.avg)} (found ${rgFiles.length} files)\n`,
    );
  }
  ...
  if (hasRipgrep && rgResult) {
    ...
    console.log(
      `    ${C.green}(${speedup.toFixed(1)}x faster, -${savingsPercent}%)${C.reset}\n`,
    );
    ...
    console.log(
      `${C.bold}${C.green}║${C.reset}  ${C.bold}INDEX BENCHMARK SUMMARY${C.reset}                              ${C.bold}${C.green}║${C.reset}`,
    );
    console.log(
      `${C.bold}${C.green}╠═══════════════════════════════════════════════════════════╣${C.reset}`,
    );
    console.log(
      `${C.bold}${C.green}║${C.reset}  Ripgrep is ${C.bold}${formatMs(savings).padStart(10)}${C.reset} faster per index build  ${C.bold}${C.green}║${C.reset}`,
    );
    console.log(
      `${C.bold}${C.green}║${C.reset}  With 60s TTL: ${C.bold}${((savings / 60000) * 100).toFixed(2)}%${C.reset} average overhead           ${C.bold}${C.green}║${C.reset}`,
    );
    console.log(
      `${C.bold}${C.green}╚═══════════════════════════════════════════════════════════╝${C.reset}`,
    );
    ...
    console.log(
      `${C.yellow}⚠ Install ripgrep for 10-50x faster indexing${C.reset}`,
    );
  }

  console.log(`\n${C.bold}${C.yellow}Recommendation:${C.reset}`);
  console.log(
    `  ${C.green}●${C.reset} Use Ripgrep index with 60s TTL for glob operations`,
  );
  console.log(
    `  ${C.green}●${C.reset} Cache index in memory to avoid redundant scans`,
  );
  console.log(
    `  ${C.green}●${C.reset} Invalidate cache on cwd change or TTL expiry`,
  );
