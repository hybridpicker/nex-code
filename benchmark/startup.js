...
function printTable(title, data, valueFn) {
  ...
  for (const [name, value] of sorted) {
    ...
    console.log(`  ${name.padEnd(35)} ${color}${formatMs(ms).padStart(10)}${C.reset} ${C.gray}${bar}${C.reset}`);
  }
}

function printSummary() {
  console.log(`\n${C.bold}${C.green}╔═══════════════════════════════════════════════════════════╗${C.reset}`);
  console.log(`${C.bold}${C.green}║${C.reset}  ${C.bold}STARTUP BENCHMARK SUMMARY${C.reset}                                ${C.bold}${C.green}║${C.reset}`);
  console.log(`${C.bold}${C.green}╠═══════════════════════════════════════════════════════════╣${C.reset}`);
  
  const total = metrics.total;
  const target = 50;
  const status = total <= target ? `${C.green}✓ PASS${C.reset}` : `${C.red}✗ FAIL${C.reset}`;
  
  console.log(`${C.bold}${C.green}║${C.reset}  Total Startup Time: ${C.bold}${formatMs(total).padStart(10)}${C.reset}  (Target: ${target}ms)  ${status}  ${C.bold}${C.green}║${C.reset}`);
  console.log(`${C.bold}${C.green}╚═══════════════════════════════════════════════════════════╝${C.reset}`);
}

async function runBenchmarks() {
  console.log(`${C.bold}${C.blue}Nex Code — Startup Benchmark${C.reset}`);
  console.log(`${C.gray}Measuring module load times and initialization phases...${C.reset}`);
  ...
  measureModuleLoad('context-engine.js', () => require('../cli/context-engine'));
  ...
  measureModuleLoad('tool-validator.js', () => require('../cli/tool-validator'));
  measureModuleLoad('tool-tiers.js', () => require('../cli/tool-tiers'));
  mark('core-modules');
  
  // Phase 2: Provider loading
  mark('providers');
  measureModuleLoad('providers/registry', () => require('../cli/providers/registry'));
  measureModuleLoad('providers/openai', () => require('../cli/providers/openai'));
  measureModuleLoad('providers/anthropic', () => require('../cli/providers/anthropic'));
  measureModuleLoad('providers/gemini', () => require('../cli/providers/gemini'));
  measureModuleLoad('providers/ollama', () => require('../cli/providers/ollama'));
  ...
  if (slowModules.length > 0) {
    console.log(`  ${C.red}●${C.reset} Slow modules (>20ms) — consider lazy loading:`);
  ...
  if (slowPhases.length > 0) {
    console.log(`  ${C.yellow}●${C.reset} Slow phases (>30ms) — optimization potential:`);
  ...
  if (metrics.total <= 50) {
    console.log(`  ${C.green}●${C.reset} Startup time is within target (<50ms) ✓`);
  }
