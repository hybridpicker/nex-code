...
const SAMPLE_MESSAGES = [
  { role: 'system', content: 'You are Nex Code, an expert coding assistant...' },
  { role: 'user', content: 'Can you help me refactor this function?' },
  {
    role: 'assistant',
    content: 'Sure! Let me first read the file to understand the current implementation.',
    tool_calls: [
      {
        id: 'call_123',
        function: {
...
async function runBenchmarks() {
  console.log(`${C.bold}${C.blue}Nex Code — Message Serialization Benchmark${C.reset}`);
  console.log(`${C.gray}Comparing cached vs uncached message formatting...${C.reset}\n`);

  const iterations = 1000;

  // Benchmark 1: Without cache (baseline)
  const baseline = benchmark('Without Cache', () => {
  ...
  const cacheMiss = benchmark('With Cache (miss)', () => {
    for (const msg of SAMPLE_MESSAGES) {
  ...
  const cacheHit = benchmark('With Cache (hit)', () => {
    for (const msg of SAMPLE_MESSAGES) {
      formatter.formatMessage(msg);
    }
  }, iterations);
  ...
  console.log(`  ${C.bold}Messages per iteration:${C.reset} ${SAMPLE_MESSAGES.length}`);
  ...
  const missImprovement = ((baseline.avg - cacheMiss.avg) / baseline.avg * 100).toFixed(1);
  console.log(`    ${missImprovement > 0 ? C.green : C.red}(${missImprovement > 0 ? '+' : ''}${missImprovement}% vs baseline)${C.reset}\n`);
  ...
  const hitImprovement = ((baseline.avg - cacheHit.avg) / baseline.avg * 100).toFixed(1);
  console.log(`    ${hitImprovement > 0 ? C.green : C.red}(${hitImprovement > 0 ? '+' : ''}${hitImprovement}% vs baseline)${C.reset}\n`);
  ...
  console.log(`${C.bold}${C.green}║${C.reset}  ${C.bold}SERIALIZATION CACHING SUMMARY${C.reset}                        ${C.bold}${C.green}║${C.reset}`);
  console.log(`${C.bold}${C.green}╠═══════════════════════════════════════════════════════════╣${C.reset}`);
  console.log(`${C.bold}${C.green}║${C.reset}  Time saved per ${iterations} iterations: ${C.bold}${formatMs(savings).padStart(10)}${C.reset}  ${C.green}(-${savingsPercent}%)${C.reset}  ${C.bold}${C.green}║${C.reset}`);
  console.log(`${C.bold}${C.green}║${C.reset}  Estimated savings per API call: ${C.bold}${formatMs(savings / iterations * SAMPLE_MESSAGES.length).padStart(6)}${C.reset}          ${C.bold}${C.green}║${C.reset}`);
  console.log(`${C.bold}${C.green}╚═══════════════════════════════════════════════════════════╝${C.reset}`);

  console.log(`\n${C.bold}${C.yellow}Recommendation:${C.reset}`);
  if (savingsPercent > 50) {
    console.log(`  ${C.green}●${C.reset} Excellent! Caching provides significant speedup (${savingsPercent}%).`);
  } else if (savingsPercent > 20) {
    console.log(`  ${C.yellow}●${C.reset} Good improvement (${savingsPercent}%). Worth implementing.`);
  } else {
    console.log(`  ${C.gray}●${C.reset} Marginal improvement (${savingsPercent}%). Consider if complexity is worth it.`);
  }
