...
const toolResultPromises = tool_calls.map((tc) => {
  ...
  return executeTool(fnName, args, { autoConfirm: true, silent: true }).then(
    (result) => ({ role: "tool", content: result, tool_call_id: callId }),
  );
});
...
_Performance Roadmap: All tasks completed ✅_
